...
public class ServiceValidate extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        try {
            ...
            if (request.getParameter("service") == null || request.getParameter("ticket") == null) {
                validationFailure(out, INVALID_REQUEST, "'service' and 'ticket' parameters are both required");
            }
            else {
                ...
                if (st == null) {
                    validationFailure(out, INVALID_TICKET, "ticket '" + ticket + "' not recognized");
                }
                else if (!st.getService().equals(service)) {
                    validationFailure(out, INVALID_SERVICE, "ticket '" + ticket + "' does not match supplied service");
                }
                else if ("true".equals(renew) && !st.isFromNewLogin()) {
                    validationFailure(out, INVALID_TICKET, "ticket not backed by initial CAS login, as requested");
                }
                else {
                    String pgtIOU = null;
    ...
    protected static void validationFailure(PrintWriter out, String code, String errorMessage) throws IOException {
    ...
    protected void validationSuccess(PrintWriter out, ServiceTicket st, String pgtIOU) {
        out.println("<cas:serviceResponse xmlns:cas='http://www.yale.edu/tp/cas'>");
        out.println("  <cas:authenticationSuccess>");
        out.println("    <cas:user>" + st.getUsername() + "</cas:user>");
        if (pgtIOU != null && !pgtIOU.equals("")) {
            out.println("    <cas:proxyGrantingTicket>" + pgtIOU + "</cas:proxyGrantingTicket>");
        }
        out.println("  </cas:authenticationSuccess>");
        out.println("</cas:serviceResponse>");
    }
    ...
    private String sendPgt(ServiceTicket st, String callbackUrl) throws TicketException {
    ...
    /**
     * Contacts the URL with a PGT and an IOU, but only if the URL's server's certificate appears appropriate for the URL. Returns
     * <tt>true</tt> on success, <tt>false</tt> on failure of any kind.
     */
    private boolean callbackWithPgt(String callbackUrl, String pgtId, String iouId) {
        try {
            String target = null;
