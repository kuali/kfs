...
/*
    ...
    public CreditMemo getCreditMemoById(Integer id) {
      LOG.debug("getCreditMemoById() started");

      Criteria criteria = new Criteria();
      criteria.addEqualTo("id",id);

      CreditMemo cm = (CreditMemo)getPersistenceBrokerTemplate().getObjectByQuery(new QueryByCriteria(CreditMemo.class,criteria));
      if ( cm != null ) {
    ...
    public CreditMemo save(CreditMemo cm,User user,boolean retrieveAllReferences) {
      LOG.debug("save() started");

      Timestamp now = new Timestamp( (new Date()).getTime() );

      cm.setLastUpdateTimestamp(now);
      ...
      for (Iterator iter = cm.getItems().iterator(); iter.hasNext();) {
        ...
        item.setLastUpdateTimestamp(now);
  ...
  /*
    public boolean duplicateExists(Integer vendorNumberHeaderId, Integer vendorNumberDetailId, 
        ...
        int cmCount = getPersistenceBrokerTemplate().getCount(new QueryByCriteria(CreditMemo.class, criteria));
        if (cmCount > 0) {
            return true;
        } else {
            return false;
  ...
  */  
  ...
  /*
    public List getCreditMemosByRequisitionId(Integer requisitionID) {
      ...
      QueryByCriteria qbc = new QueryByCriteria(CreditMemo.class,criteria);
