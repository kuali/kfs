...
<div class="tab-container" align=center>
  ...
  <table>
    <tbody>
      <tr>
        ...
        <kul:htmlAttributeLabel attributeEntry="${cashReceiptAttributes.totalCheckAmount}" useShortLabel="false" /></div></th>
	        ...
	        <c:if test="${!readOnly}">
	        	...
	        	<c:if test="${checkDetailMode}"> $${KualiForm.document.currencyFormattedTotalCheckAmount} 
	        ...
	        <c:if test="${!readOnly}">
	        		...
	        		<html:select property="checkEntryMode" onchange="submitForm()" > <html:optionsCollection property="checkEntryModes" label="label" value="value" /> </html:select>
		          <noscript>
      ...
      <tr>
        <th><div align="right"><strong><kul:htmlAttributeLabel attributeEntry="${cashReceiptAttributes.totalCashAmount}" useShortLabel="false" /></strong></div></th>
        <td width="35%" align="left" valign="middle"><c:if test="${readOnly}"> $${KualiForm.document.currencyFormattedTotalCashAmount} <html:hidden write="false" property="document.totalCashAmount" /> </c:if> <c:if test="${!readOnly}"> <kul:htmlControlAttribute property="document.totalCashAmount" attributeEntry="${cashReceiptAttributes.totalCashAmount}" /> </c:if> </td>
      </tr>
      <tr>
        <th><div align="right"><strong><kul:htmlAttributeLabel attributeEntry="${cashReceiptAttributes.totalCoinAmount}" useShortLabel="false" /></strong></div></th>
        <td width="35%" align="left" valign="middle"><c:if test="${readOnly}"> $${KualiForm.document.currencyFormattedTotalCoinAmount} <html:hidden write="false" property="document.totalCoinAmount" /> </c:if> <c:if test="${!readOnly}"> <kul:htmlControlAttribute property="document.totalCoinAmount" attributeEntry="${cashReceiptAttributes.totalCoinAmount}" /> </c:if> </td>
      </tr>
      <tr>
        <th><div align="right"><strong>Cash Reconciliation Total:</strong></div></th>
        <td width="35%" align="left" valign="middle">$${KualiForm.document.currencyFormattedSumTotalAmount}&nbsp;&nbsp;&nbsp; <c:if test="${!readOnly}"> <html:image src="images/tinybutton-recalculate.gif" styleClass="tinybutton" alt="recalculate total" /> </c:if> <c:if test="${readOnly}"> &nbsp; </c:if></td>
      </tr>
    </tbody>
  </table>
</div>
