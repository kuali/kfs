...
public class DepreciableAssetsDaoOjb extends PlatformAwareDaoBaseOjb implements DepreciableAssetsDao {
    ...
    private Integer getFullyDepreciatedAssetCount() {
        ...
        q.setAttributes(new String[] { PAYMENT_TO_ASSET_REFERENCE_DESCRIPTOR + CamsPropertyConstants.Asset.SALVAGE_AMOUNT, "SUM(" + CamsPropertyConstants.AssetPayment.PRIMARY_DEPRECIATION_BASE_AMOUNT + ")", "SUM(" + CamsPropertyConstants.AssetPayment.ACCUMULATED_DEPRECIATION_AMOUNT + ")", CamsPropertyConstants.AssetPayment.CAPITAL_ASSET_NUMBER });
        ...
        while (i.hasNext()) {
            ...
            if (baseAmount.subtract(salvageAmount).compareTo(accumulatedDepreciation) == 0) {
                System.out.println(fieldValue[3] + "-" + baseAmount + "-" + salvageAmount + "-" + accumulatedDepreciation);
