...
public class ExtractDaoOjb extends PlatformAwareDaoBaseOjb implements ExtractDao {
    ...
    public Collection<Entry> findMatchingGLEntries(GeneralLedgerEntry currentEntry) {
        ...
        criteria.addEqualTo(CabPropertyConstants.GeneralLedgerEntry.CHART_OF_ACCOUNTS_CODE, currentEntry.getChartOfAccountsCode());
        criteria.addEqualTo(CabPropertyConstants.GeneralLedgerEntry.ACCOUNT_NUMBER, currentEntry.getAccountNumber());
        criteria.addEqualTo(CabPropertyConstants.GeneralLedgerEntry.FINANCIAL_OBJECT_CODE, currentEntry.getFinancialObjectCode());
        criteria.addEqualTo(CabPropertyConstants.GeneralLedgerEntry.UNIVERSITY_FISCAL_YEAR, currentEntry.getUniversityFiscalYear());
        criteria.addEqualTo(CabPropertyConstants.GeneralLedgerEntry.UNIVERSITY_FISCAL_PERIOD_CODE, currentEntry.getUniversityFiscalPeriodCode());
        criteria.addNotEqualTo(CabPropertyConstants.GeneralLedgerPendingEntry.FINANCIAL_BALANCE_TYPE_CODE, KFSConstants.SubAccountType.EXPENSE);

        if (StringUtils.isNotBlank(currentEntry.getOrganizationReferenceId())) {
            criteria.addEqualTo(CabPropertyConstants.GeneralLedgerEntry.ORGNIZATION_REFERENCE_ID, currentEntry.getOrganizationReferenceId());
        }

        criteria.addEqualTo(CabPropertyConstants.GeneralLedgerEntry.PROJECT_CD, currentEntry.getProjectCode());
