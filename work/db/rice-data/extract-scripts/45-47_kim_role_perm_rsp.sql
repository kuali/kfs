...
SELECT 'KFS'||ROLE_PERM_ID AS ROLE_PERM_ID, 'KFS'||ROLE_PERM_ID AS OBJ_ID, ROLE_ID, PERM_ID, ACTV_IND 
...
SELECT 'KFS'||ROLE_RSP_ID AS ROLE_RSP_ID, 'KFS'||ROLE_RSP_ID AS OBJ_ID, ROLE_ID, RSP_ID, ACTV_IND 
...
SELECT 'KFS'||ROLE_RSP_ACTN_ID AS ROLE_RSP_ACTN_ID, 'KFS'||ROLE_RSP_ACTN_ID AS OBJ_ID, ACTN_TYP_CD, NVL( PRIORITY_NBR, 1 ) AS PRIORITY_NBR, ACTN_PLCY_CD, 
        CASE WHEN NVL( role_mbr_id, '*' ) != '*' THEN 'KFS'||ROLE_MBR_ID ELSE role_mbr_id END AS ROLE_MBR_ID, 
        CASE WHEN NVL( role_rsp_id, '*' ) != '*' THEN 'KFS'||role_rsp_id ELSE role_rsp_id END AS ROLE_RSP_ID, FRC_ACTN 
