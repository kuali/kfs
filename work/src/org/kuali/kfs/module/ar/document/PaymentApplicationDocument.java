...
public class PaymentApplicationDocument extends GeneralLedgerPostingDocumentBase implements GeneralLedgerPendingEntrySource, Correctable, Copyable {
    ...
    public void doRouteStatusChange(DocumentRouteStatusChangeDTO statusChangeEvent) {
        ...
        if (this.isPaymentApplicationCorrection() && getDocumentHeader().getWorkflowDocument().stateIsSaved()) {
            if (cashControlDetail != null) {
    ...
    /**
     * @see org.kuali.rice.kns.document.DocumentBase#setNewDocumentHeader()
     */
    @Override
    protected void setNewDocumentHeader() throws WorkflowException {
        PaymentApplicationDocument newDoc = (PaymentApplicationDocument) KNSServiceLocator.getDocumentService().getNewDocument(getDocumentHeader().getWorkflowDocument().getDocumentType());
        ...
        try {
            setObjectPropertyDeep(this, KNSPropertyConstants.DOCUMENT_NUMBER, documentNumber.getClass(), newDoc.getDocumentNumber());
        }
        catch (Exception e) {
     ...
     * from org.kuali.rice.kns.util.ObjectUtils
     */
    /**
     * 
     */
        ...
        for (int i = 0; i < propertyDescriptors.length; i++) {
            ...
            if (propertyDescriptor.getPropertyType() != null && (BusinessObject.class).isAssignableFrom(propertyDescriptor.getPropertyType()) && PropertyUtils.isReadable(bo, propertyDescriptor.getName())) {
                ...
                if (nestedBo instanceof BusinessObject) {
                    ...
                    if ("InvoicePaidApplied".equals(propertyDescriptor.getName())) {
                        System.out.println("\n\n\t\tTHai this is invoice paid applied.");
