...
public class DisbursementVoucherAction extends KualiAccountingDocumentActionBase {
    ...
    protected ActionForward refreshAfterPayeeSelection(ActionMapping mapping, DisbursementVoucherForm dvForm, HttpServletRequest request) {
        ...
        if (isPayeeLookupable && dvForm.isCustomer()) {
            AccountsReceivableCustomer customer = (AccountsReceivableCustomer) 
                                             SpringContext.getBean(AccountsReceivableModuleService.class).findCustomer(payeeIdNumber);
    ...
    protected ActionForward renderCustomerAddressSelection(ActionMapping mapping, HttpServletRequest request, DisbursementVoucherForm dvForm) {
        ...
        props.put(KNSConstants.SUPPRESS_ACTIONS, Boolean.toString(true));
        props.put(KNSConstants.BUSINESS_OBJECT_CLASS_ATTRIBUTE, AccountsReceivableCustomerAddress.class.getName());
        props.put(KNSConstants.LOOKUP_ANCHOR, KNSConstants.ANCHOR_TOP_OF_FORM);
        props.put(KNSConstants.LOOKED_UP_COLLECTION_NAME, KFSPropertyConstants.VENDOR_ADDRESSES);
    ...
    protected void setupPayeeAsCustomer(DisbursementVoucherForm dvForm, String payeeIdNumber, String payeeAddressIdentifier) {
        ...
        if (StringUtils.isNotBlank(payeeAddressIdentifier)) {
            customerAddress = (AccountsReceivableCustomerAddress) 
               SpringContext.getBean(AccountsReceivableModuleService.class).findCustomerAddress(payeeIdNumber, payeeAddressIdentifier);
        }
