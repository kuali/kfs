              ...
              <c:if test="${person.personToBeNamedIndicator}">TO BE NAMED</c:if>
                    <c:if test="${!viewOnly}">
                      <c:choose>
                        ...
                        <c:otherwise>
                          <html:text title="* Name" property="document.routingFormPersonnel[${status.index}].principalName" onblur="personIDLookup('document.routingFormPersonnel[${status.index}].principalName')"/>                  
                          <kul:lookup boClassName="org.kuali.rice.kim.bo.Person" fieldConversions="principalId:document.routingFormPersonnel[${status.index}].principalId,name:document.routingFormPersonnel[${status.index}].user.principalName" extraButtonSource="${ConfigProperties.externalizable.images.url}buttonsmall_namelater.gif" extraButtonParams="&document.routingFormPersonnel[${status.index}].personToBeNamedIndicator=true" anchor="${currentTabIndex}" />
                        </c:otherwise>
                      </c:choose>
                    </c:if>
		          <div id="document.routingFormPersonnel[${status.index}].user.name.div" >
