...
public class GenesisDaoOjb extends PlatformAwareDaoBaseOjb implements GenesisDao {
    /*
     * These routines are written to try to mitigate the performance hit that comes from using OJB as opposed to JDBC (pass-through
     * SQL). Pass-through SQL in Kuali could lead to database-dependencies in the code, and tie Kuali to a specific RDBMS. OJB is
     * not really suited for batch, where rows are fetched, inserted, and updated in big bunches as opposed to a few at a time. (1)
     * OJB in "lazy evaluation mode" (the Kuali standard for performance reasons) will only return the row from the main table
     * regardless of how many "reference descriptor" joins and/or "collection descriptor" joins there may be in the OJB repository
     * file. So, if I query table A and reference table B, my query (in batch) might return 10,000 A rows in a single call. None of
     * the matching B fields will be filled in in the DAO. If I then try to access a B field in a given instance of the DAO, Spring
     * will do a query to fetch the relevant B row. In essence, in batch I would do a single DB call to get the 10,000 rows of A,
     * and 10,000 DB calls to fill in the fields from B, one for each row of A. (2) This routine tries to do joins in java, in
     * memory, by using what Oracle calls a "hash join". If we want to join A and B on a key, we will get the relevant fields from A
     * and B on separate DB calls (one for A and one for B), and create a hash map on the join key from the results. We can then
     * iterate through either A or B and get the relevant fields from the other table by employing the hash key. This should be
     * fast, since hash tables are designed for fast access. (3) We will only store when absolutely necessary to minimize data base
     * access. So where in Oracle we would do an UPDATE A.. WHERE (EXISTS (SELECT 1 FROM B WHERE A matches B) or an INSERT A (SELECT
     * ... FROM A, B WHERE A = B), we will get all the candidate rows from both A and B, and store individually to do INSERT or
     * UPDATE. (There seems to be now way in OJB to store more than one row at a time.) This may lead to a lot of database calls
     * that operate on a single row. We can only try to minimize this problem. We can't get around it. This is the impression of the
     * coder. If anyone has other suggestions, please let us know. (One alternative might be to have many different class-desriptor
     * tags in the OJB repository file representing table A, one for each join to table B. If we could override lazy evaluation at
     * the class-descriptor level, we could code some batch-specific joins that would get everything we need in one call. The
     * problem with this is that the A/B descriptions would then be in multiple tags, and changing them would be labor-intensive and
     * error-prone. But OJB repositories allow headers, so we could get around this by using an entity to describe the A fields. The
     * entity would be in one place, so changes to the A fields could also be made in one place. The foreignkey field-ref tag B
     * fields are repeated in every description anyway, so things aren't always in one place to begin with.)
     */
    ...
    public final Map<String, String> getBudgetConstructionControlFlags(Integer universityFiscalYear) {
        ...
        /*
         * the key to the map returned will be the name of the flag the entry will be the flag's value
         */
    ...
    /*
     * ****************************************************************************** These are utility routines used by all the
     * units ******************************************************************************
     */
    ...
    private Integer hashObjectSize(Class classID, Criteria criteriaID, String propertyName) {
        // this one counts distinct rows
        String[] selectList = new String[] { "COUNT(DISTINCT " + propertyName + ")" };
        ReportQueryByCriteria queryID = new ReportQueryByCriteria(classID, selectList, criteriaID);
        Iterator resultRows = getPersistenceBrokerTemplate().getReportQueryIteratorByQuery(queryID);
        if (resultRows.hasNext()) {
            return (hashCapacity(((BigDecimal) ((Object[]) TransactionalServiceUtils.retrieveFirstAndExhaustIterator(resultRows))[0]).intValue()));
    ...
    private Integer hashObjectSize(Class classID, Criteria criteriaID, String[] selectList) {
    ...
    /*
     * ******************************************************************************** This routine returns the fiscal year as of
     * the run date. Normally, genesis runs in the current fiscal year to build budget construction for the coming fiscal year.
     */
    ...
    /*
     * ****************************************************************************** (1) these routines are used to create and set
     * the control flags for budget * construction * ******************************************************************************
     */
    ...
    /*
     * **************************************************************** (2) intialization for genesis * these methods clean out the
     * PBGL and document tables. * BC only allows one fiscal year at a time * (this could be modified to clear things out by fiscal
     * year) * it should be modified to add more tables * * NOTE (IMPORTANT): * In order to do a bulk delete, we MUST use
     * deleteByQuery. The * only alternative is to fetch each existing row and delete it, * one at a time. This is unrealistic in a
     * batch process dealing * with many rows. deleteByQuery does NOT "synchronize" the cache:* the deleted rows remain in the cache
     * and will be fetched on a * subsequent database call if they fit the criteria. The OJB * documentation explicitly states this.
     * So, after a * deleteByQuery we need to call clearCache. This will not remove* instantiated objects but will remove the
     * database rows that * were used to build them. One consequence is that every store * of an object that was instantiated before
     * the cache was cleared* will generate a select on the first key field (to test whether * the object exists) and then an INSERT
     * if it does not or an * UPDATE if it does. This same process happens if on changes the* key (for example, the Fiscal Year) of
    ...
    /*
     * @@TODO: this is prior to October, 2007, when calling complete stopped running in finite time private void
     * finalizeBCDocument(BudgetConstructionDocument bcDoc) throws WorkflowException { // this routine reads the actual workflow
     * tables to set the status dates and the final status code // going through the routing procedure took far too much time per
     * document DocumentRouteHeaderValue routeHeader =
     * routeHeaderService.getRouteHeader(bcDoc.getDocumentHeader().getWorkflowDocument().getRouteHeaderId());
     * routeHeader.markDocumentEnroute(); routeHeader.markDocumentApproved(); routeHeader.markDocumentProcessed();
     * routeHeader.markDocumentFinalized(); routeHeaderService.saveRouteHeader(routeHeader); }
     */
    ...
    private void getCurrentBCHeaderKeys(Integer BaseYear) {
        ...
        while (ReturnedRows.hasNext()) {
            BudgetConstructionHeader bCHdr = (BudgetConstructionHeader) ReturnedRows.next();
            currentBCHeaderKeys.add(bCHdr.getChartOfAccountsCode() + bCHdr.getAccountNumber() + bCHdr.getSubAccountNumber());
        }
    }
    ...
    public void setUpCurrentBCHeaderKeys(Integer BaseYear) {
        ...
        Criteria criteriaID = new Criteria();
        criteriaID.addEqualTo(KFSPropertyConstants.UNIVERSITY_FISCAL_YEAR, BaseYear);
        criteriaID.addEqualTo(KFSPropertyConstants.BALANCE_TYPE_CODE, BudgetConstructionConstants.BUDGET_CONSTRUCTION_DOCUMENT_TYPE);
        String[] propertyString = { KFSPropertyConstants.CHART_OF_ACCOUNTS_CODE, KFSPropertyConstants.ACCOUNT_NUMBER, KFSPropertyConstants.SUB_ACCOUNT_NUMBER };
    ...
    public void storeANewBCDocument(BudgetConstructionDocument newBCHdr) throws WorkflowException {
        ...
        kualiDocumentHeader.setFinancialDocumentDescription(String.format("%s %d %s %s", BudgetConstructionConstants.BUDGET_CONSTRUCTION_DOCUMENT_DESCRIPTION, newBCHdr.getUniversityFiscalYear(), newBCHdr.getChartOfAccountsCode(), newBCHdr.getAccountNumber()));
    ...
    /*
     * **************************************************************************** (4) here are the routines which freeze
     * accounting at the beginning of * budget construction (so updates can be done in parallel, or updates * for the budget year
     * only can be done without affecting the current * chart of accounts). * These routines only run once, at genesis. *
     * ****************************************************************************
     */
    ...
    private void buildNewAccountReportsTo() {
        ...
        Criteria criteriaID = new Criteria();
        /*
         * current IU genesis does NOT check for closed accounts--it loads all accounts it is possible that an account which has
         * been closed still has base budget criteriaID.addNotEqualTo(KFSPropertyConstants.ACCOUNT_CLOSED_INDICATOR,
    ...
    private void buildNewOrganizationReportsTo() {
        ...
        Criteria criteriaID = new Criteria();
        /*
         * IU genesis takes all organizations, not just active ones the reason is that a closed account which still has a base
         * budget might report to one of these organizations
         * criteriaID.addEqualTo(KFSPropertyConstants.ORGANIZATION_ACTIVE_INDICATOR, KFSConstants.ParameterValues.YES);
         */
        criteriaID = QueryByCriteria.CRITERIA_SELECT_ALL;
    ...
    /*
     * ********************************************************************************* (5) these are the routines that build the
     * security organization hierarchy -- they run every time the budget construction update process runs -- they are designed to
     * pick up any changes made to the BC account and BC organization tables -- based on changes, they will adjust the security
     * levels of accounts in the BC header. for a header at the level of an organization that is no longer valid, the level will
     * return to the account manager level. for a header at the level of an organization that has changed its location in the
     * hierarchy, the new level will be added to the header -- this process only affects accounts in the budget construction pending
     * general ledger, and it is assumed that all updates to the PBGL have been finished when this process runs.
     * *********************************************************************************
     */
    ...
    /*
     * ************************************************************************** (6) here are the routines we will use for updating
     * budget construction GL* **************************************************************************
     */
    ...
    private PendingBudgetConstructionGeneralLedger newPBGLBusinessObject(Integer RequestYear, Object[] sqlResult) {
        ...
        /*
         * the document number will be set later if we have to store this in a new document a new row in an existing document will
         * take it's document number from the existing document otherwise (existing document, existing row), the only field in this
         * that will be the beginning balance amount
         */
    ...
    /*
     * ****************************************************************************** (7) there could be an object class in the
     * object code table that was marked as inactive during the current fiscal year. there could also be GL rows with base budget
     * which refer to this object code. the fiscal year makers routine would NOT copy a deleted object code into the new fiscal
     * year. to maintain referential integrity, we will copy such an object code (but mark it as deleted) into the new fiscal year
     * if it will occur in budget construction.
     */
    ...
    /*******************************************************************************************************************************
     * (8) the budget construction position table is dependent on each institution's payroll system, and thus cannot be coded
     * effectively in Kuali. However, RI in the database requires that it be populated. This is a stub routine to do so.
     * *******************************************************************************************
     */
    private HashMap<String, BudgetConstructionPosition> currentBCPosition = new HashMap<String, BudgetConstructionPosition>(1);
    private HashSet<String> deletedCSFOverridePositions = new HashSet();
    ...
    private String buildDeletedKey(Object[] returnedRow) {
        return ((String) returnedRow[returnedPositionNumber]) + vacantEmplid((String) returnedRow[returnedCSFFundingStatusCode], (String) returnedRow[returnedEmplid]) + ((String) returnedRow[returnedChartOfAccountsCode]) + ((String) returnedRow[returnedAccountNumber]) + ((String) returnedRow[returnedSubAccountNumber]) + ((String) returnedRow[returnedFinancialObjectCode]) + ((String) returnedRow[returnedFinancialSubObjectCode]);
    ...
    private void buildDeletedList(Integer BaseYear) {
        ...
        Criteria criteriaID = new Criteria();
        criteriaID.addEqualTo(KFSPropertyConstants.UNIVERSITY_FISCAL_YEAR, BaseYear);
        criteriaID.addNotEqualTo(KFSPropertyConstants.CSF_DELETE_CODE, BudgetConstructionConstants.ACTIVE_CSF_LINE);
        String[] selectList = buildCSFPositionSelectList();
        ReportQueryByCriteria queryID = new ReportQueryByCriteria(CalculatedSalaryFoundationTrackerOverride.class, selectList, criteriaID);
        ...
        LOG.info(String.format("\ndeleted CSF Override rows %d", deletedCSFOverridePositions.size()));
    }

    private String[] buildCSFPositionSelectList() {
        String[] returnArray = { KFSPropertyConstants.POSITION_NUMBER, KFSPropertyConstants.EMPLID, KFSPropertyConstants.CHART_OF_ACCOUNTS_CODE, KFSPropertyConstants.ACCOUNT_NUMBER, KFSPropertyConstants.SUB_ACCOUNT_NUMBER, KFSPropertyConstants.FINANCIAL_OBJECT_CODE, KFSPropertyConstants.FINANCIAL_SUB_OBJECT_CODE, KFSPropertyConstants.CSF_FUNDING_STATUS_CODE };
        return returnArray;
    }
    ...
    private void buildNewPositionFromCSF(Integer FiscalYear, Object[] rowReturned) {
        ...
        currentBCPosition.put(positionKey(FiscalYear, (String) rowReturned[returnedPositionNumber]), newBCPosition);
    }
    ...
    private Integer countCSFPositions(Integer FiscalYear) {
        ...
        criteriaID.addEqualTo(KFSPropertyConstants.CSF_DELETE_CODE, BudgetConstructionConstants.ACTIVE_CSF_DELETE_CODE);
        return (hashObjectSize(CalculatedSalaryFoundationTracker.class, criteriaID, KFSPropertyConstants.POSITION_NUMBER));
    }
    ...
    private void readCSFOverridePositions(Integer BaseYear, boolean CSFUpdatesAllowed) {
        ...
        Criteria criteriaID = new Criteria();
        criteriaID.addEqualTo(KFSPropertyConstants.UNIVERSITY_FISCAL_YEAR, BaseYear);
        criteriaID.addEqualTo(KFSPropertyConstants.CSF_DELETE_CODE, BudgetConstructionConstants.ACTIVE_CSF_LINE);
        String[] selectList = buildCSFPositionSelectList();
        ReportQueryByCriteria queryID = new ReportQueryByCriteria(CalculatedSalaryFoundationTrackerOverride.class, selectList, criteriaID);
        Iterator resultSet = getPersistenceBrokerTemplate().getReportQueryIteratorByQuery(queryID);
        while (resultSet.hasNext()) {
            ...
            if (!CSFUpdatesAllowed) {
                String testBaseKey = positionKey(BaseYear, (String) rowReturned[returnedPositionNumber]);
        ...
        LOG.info(String.format("\noverride rows (1) read = %d\n" + "             (2) base year positions = %d\n" + "             (3) request year positions = %d", overridesRead, overridePositions, overrideRequest));
    }
    ...
    private void readCSFPositions(Integer BaseYear, boolean CSFUpdatesAllowed) {
        ...
        LOG.info(String.format("\nCSF rows (1) read = %d\n" + "        (2) base year positions = %d\n" + "        (3) request year positions = %d", csfRead, csfPositions, csfRequest));
    }

    private void readCurrentPositions(Integer BaseYear, boolean CSFUpdatesAllowed) {
        ...
        if (CSFUpdatesAllowed) {
            ...
            Criteria criteriaID = new Criteria();
            criteriaID.addBetween(KFSPropertyConstants.UNIVERSITY_FISCAL_YEAR, BaseYear, RequestYear);
            queryId = new QueryByCriteria(BudgetConstructionPosition.class, criteriaID);
            // CSF could create a base and a request position
            positionHashSize = hashObjectSize(BudgetConstructionPosition.class, criteriaID) + 2 * countCSFPositions(BaseYear);
        }
        else {
            ...
            positionHashSize = hashObjectSize(BudgetConstructionPosition.class, criteriaID) + countCSFPositions(BaseYear);
            queryId = new QueryByCriteria(BudgetConstructionPosition.class, criteriaID);
        }
        currentBCPosition = new HashMap<String, BudgetConstructionPosition>(positionHashSize);
        Iterator rowsReturned = getPersistenceBrokerTemplate().getIteratorByQuery(queryId);
        while (rowsReturned.hasNext()) {
            BudgetConstructionPosition currentPosition = (BudgetConstructionPosition) rowsReturned.next();
            String hashKey = positionKey(currentPosition.getUniversityFiscalYear(), currentPosition.getPositionNumber());
            currentBCPosition.put(hashKey, currentPosition);
        }
        LOG.info(String.format("\nposition count before updates %d", currentBCPosition.size()));
    }

    private void storeNewPositions() {
        Integer positionsWritten = new Integer(0);
        for (Map.Entry<String, BudgetConstructionPosition> positionRows : currentBCPosition.entrySet()) {
            BudgetConstructionPosition testPosition = positionRows.getValue();
            if (!(storeNewPositionsCriteria(testPosition))) {
                ...
                testPosition.setPositionLockUserIdentifier(BudgetConstructionConstants.DEFAULT_BUDGET_HEADER_LOCK_IDS);
                getPersistenceBrokerTemplate().store(testPosition);
                positionsWritten = positionsWritten + 1;
            }
        }
        LOG.info(String.format("\n%d of %d budget positions written", positionsWritten, currentBCPosition.size()));
    }
    ...
    private boolean storeNewPositionsCriteria(BudgetConstructionPosition testPosition) {
        ...
        boolean nullDefaultLock = (BudgetConstructionConstants.DEFAULT_BUDGET_HEADER_LOCK_IDS == null);
        ...
        if ((nullDefaultLock) || (nullLock)) {
            ...
            return (testPosition.getPositionLockUserIdentifier().compareTo(BudgetConstructionConstants.DEFAULT_BUDGET_HEADER_LOCK_IDS) == 0);
        }
    }

    private String vacantEmplid(String vacantCSFCode, String emplidCSF) {
        return (vacantCSFCode.equals(BudgetConstructionConstants.VACANT_CSF_LINE) ? BudgetConstructionConstants.VACANT_EMPLID : emplidCSF);
    }
    ...
    private void buildRequestYearSpoof(Integer RequestYear, Object[] rowReturned) {
        String testRequestKey = positionKey(RequestYear, (String) rowReturned[returnedPositionNumber]);
    ...
    private java.sql.Date defaultAugustFirst(Integer dateYear) {
        ...
        try {
            java.sql.Date augustFirst = dateTimeService.convertToSqlDate(dateYear.toString() + "-08-01");
            return augustFirst;
        }
        catch (ParseException ex) {
            LOG.warn("\nproblem setting July 1 position date with: (" + dateYear.toString() + "-08-01)\ncurrent date was used\n" + ex.getMessage());
    ...
    private java.sql.Date defaultJulyFirst(Integer dateYear) {
        // this routine assumes a specific SQLDATE_FORMAT in the date service
        try {
            java.sql.Date julyFirst = dateTimeService.convertToSqlDate(dateYear.toString() + "-07-01");
            return julyFirst;
        }
        catch (ParseException ex) {
            LOG.warn("\nproblem setting July 1 position date with: (" + dateYear.toString() + "-07-01)\ncurrent date was used\n" + ex.getMessage());
            return dateTimeService.getCurrentSqlDateMidnight();
        }
    }

    private void fillPositionAttributesSpoof(BudgetConstructionPosition newBCPosition, Object[] rowReturned, Integer FiscalYear) {
        ...
        String posTypeString = positionTypeSpoof((String) rowReturned[returnedFinancialObjectCode]);
        ...
        String acctTestKey = acctRCKey((String) rowReturned[returnedChartOfAccountsCode], (String) rowReturned[returnedAccountNumber]);
        //
        ...
        if (acctRCMap.containsKey(acctTestKey)) {
            String[] acctRptsData = acctRCMap.get(acctTestKey);
    ...
    private String positionTypeSpoof(String objectClass) {
        ...
        return ((empTypeID.equals("24")) ? "SM" : ((empTypeID.equals("25")) ? "SB" : "AC"));
    }

    private void readAcctRCCSF(Class csfClass, Integer BaseYear) {
        Criteria criteriaID = new Criteria();
        criteriaID.addEqualTo(KFSPropertyConstants.UNIVERSITY_FISCAL_YEAR, BaseYear);
        String[] selectList = { KFSPropertyConstants.CHART_OF_ACCOUNTS_CODE, KFSPropertyConstants.ACCOUNT_NUMBER };
        ReportQueryByCriteria queryID = new ReportQueryByCriteria(csfClass, selectList, criteriaID, true);
        Iterator resultSet = getPersistenceBrokerTemplate().getReportQueryIteratorByQuery(queryID);
        while (resultSet.hasNext()) {
            Object[] rowReturned = (Object[]) resultSet.next();
            String acctKey = acctRCKey((String) rowReturned[0], (String) rowReturned[1]);
    ...
    private Integer readAcctRCMapCount(Integer BaseYear) {
        ...
        criteriaID.addEqualTo(KFSPropertyConstants.UNIVERSITY_FISCAL_YEAR, BaseYear);
        String[] selectList = { "COUNT(DISTINCT " + KFSPropertyConstants.ACCOUNT_NUMBER + ")" };
        ReportQueryByCriteria queryID = new ReportQueryByCriteria(CalculatedSalaryFoundationTrackerOverride.class, selectList, criteriaID);
        Iterator ovrdIter = getPersistenceBrokerTemplate().getReportQueryIteratorByQuery(queryID);
        if (ovrdIter.hasNext()) {
            BigDecimal resultValue = (BigDecimal) ((Object[]) TransactionalServiceUtils.retrieveFirstAndExhaustIterator(ovrdIter))[0];
            acctCount = acctCount + (Integer) resultValue.intValue();
        }
        ReportQueryByCriteria queryCSF = new ReportQueryByCriteria(CalculatedSalaryFoundationTracker.class, selectList, criteriaID);
        Iterator csfIter = getPersistenceBrokerTemplate().getReportQueryIteratorByQuery(queryCSF);
        if (csfIter.hasNext()) {
    ...
    private void readAcctRCMap(Integer BaseYear) {
        ...
        // RC code
        Criteria criteriaID = new Criteria();
        criteriaID.addEqualTo(KFSPropertyConstants.ACCOUNT_CLOSED_INDICATOR, false);
        // we cannot return a business object with a report query
        // so, we just return what we need
        String[] attrbList = { KFSPropertyConstants.CHART_OF_ACCOUNTS_CODE, KFSPropertyConstants.ACCOUNT_NUMBER, KFSPropertyConstants.ORGANIZATION_CODE };
        Integer chartIndex = new Integer(0);
        Integer accountIndex = new Integer(1);
        Integer organizationIndex = new Integer(2);
        ReportQueryByCriteria queryID = new ReportQueryByCriteria(org.kuali.module.chart.bo.Account.class, attrbList, criteriaID);
        Iterator accountsReturned = getPersistenceBrokerTemplate().getReportQueryIteratorByQuery(queryID);
        while (accountsReturned.hasNext()) {
            Object[] rowReturned = (Object[]) accountsReturned.next();
            String orgPositionKey = positionOrg((String) rowReturned[chartIndex], (String) rowReturned[organizationIndex]);
            String acctKey = acctRCKey((String) rowReturned[chartIndex], (String) rowReturned[accountIndex]);
            if ((acctRCMap.containsKey(acctKey)) && (orgRCMap.containsKey(orgPositionKey))) {
                String[] newValue = { orgPositionKey, orgRCMap.get(orgPositionKey) };
    ...
    private void readOrgRCMap() {
        String[] selectList = { KFSPropertyConstants.CHART_OF_ACCOUNTS_CODE, KFSPropertyConstants.ORGANIZATION_CODE, KFSPropertyConstants.RESPONSIBILITY_CENTER_CODE };
        ReportQueryByCriteria queryID = new ReportQueryByCriteria(Org.class, selectList, QueryByCriteria.CRITERIA_SELECT_ALL);
        orgRCMap = new HashMap<String, String>(hashCapacity((Integer) getPersistenceBrokerTemplate().getCount(queryID)));
        Iterator resultSet = getPersistenceBrokerTemplate().getReportQueryIteratorByQuery(queryID);
        while (resultSet.hasNext()) {
            Object[] rowReturned = (Object[]) resultSet.next();
            String orgKey = positionOrg((String) rowReturned[0], (String) rowReturned[1]);
            orgRCMap.put(orgKey, (String) rowReturned[2]);
        }
    }

    /*******************************************************************************************************************************
     * (9) this code builds the budget construction CSF tracker and the budget construction appointment funding
     * ***************************************************************************************
     */
    ...
    public void buildAppointmentFundingAndBCSF(Integer BaseYear) {
        /***************************************************************************************************************************
         * RI requirements: this method assumes that ALL budget construction positions for the request year are in the position
         * table, and that budget construction documents for every accounting key in the CSF tables have been created
         **************************************************************************************************************************/
    ...
    private void buildAndStoreBCSFfromCSF(CalculatedSalaryFoundationTrackerOverride csf, String csfKey) {
        ...
        csfBC.setEmplid((vacantLine ? BudgetConstructionConstants.VACANT_EMPLID : csf.getEmplid()));
    ...
    private String buildVacantCSFKey(CalculatedSalaryFoundationTrackerOverride csf) {
        boolean vacantLine = isVacantLine(csf);
        return (vacantLine ? BudgetConstructionConstants.VACANT_EMPLID : csf.getEmplid()) + csf.getPositionNumber() + csf.getAccountNumber() + csf.getChartOfAccountsCode() + csf.getSubAccountNumber() + csf.getFinancialObjectCode() + csf.getFinancialSubObjectCode();
    }
    ...
    private ArrayList<String> findPositonRequiredObjectCodes(Integer BaseYear) {
        ...
        ReportQueryByCriteria queryID = new ReportQueryByCriteria(LaborObject.class, selectList, criteriaID, true);
    ...
    private boolean isVacantLine(CalculatedSalaryFoundationTracker csf) {
        return ((csf.getCsfFundingStatusCode().equals(BudgetConstructionConstants.VACANT_CSF_LINE)) || (csf.getCsfFundingStatusCode().equals(BudgetConstructionConstants.UNFUNDED_CSF_LINE)));
    }
    ...
    private void setUpBCSFMap(Integer BaseYear) {
        ...
        criteriaID.addEqualTo(KFSPropertyConstants.UNIVERSITY_FISCAL_YEAR, BaseYear);
        criteriaID.addEqualTo(KFSPropertyConstants.CSF_DELETE_CODE, BudgetConstructionConstants.ACTIVE_CSF_DELETE_CODE);
        bCSFSize = hashObjectSize(CalculatedSalaryFoundationTrackerOverride.class, criteriaID) + hashObjectSize(CalculatedSalaryFoundationTracker.class, criteriaID);
    ...
    private void setUpCSFOverrideKeys(Integer BaseYear) {
        ...
        vacantCriteria.addEqualTo(KFSPropertyConstants.CSF_FUNDING_STATUS_CODE, BudgetConstructionConstants.VACANT_CSF_LINE);
        ...
        while (csfOvrd.hasNext()) {
            csfOverrideKeys.add(buildCSFKey((CalculatedSalaryFoundationTrackerOverride) csfOvrd.next()));
        }
    ...
    private void setUpCurrentPBGLKeys(Integer BaseYear) {
        ...
        Criteria criteriaID = new Criteria();
        criteriaID.addEqualTo(KFSPropertyConstants.UNIVERSITY_FISCAL_YEAR, RequestYear);
        criteriaID.addIn(KFSPropertyConstants.FINANCIAL_OBJECT_CODE, this.findPositonRequiredObjectCodes(BaseYear));
    ...
    private void setUpKeysNeedingRounding(Integer BaseYear) {
        ...
        criteriaID.addEqualTo(KFSPropertyConstants.CSF_DELETE_CODE, BudgetConstructionConstants.ACTIVE_CSF_DELETE_CODE);
        criteriaID.addEqualTo(KFSPropertyConstants.UNIVERSITY_FISCAL_YEAR, BaseYear);
        keysNeedingRounding = new HashMap<String, roundMechanism>(hashObjectSize(CalculatedSalaryFoundationTrackerOverride.class, criteriaID, KFSPropertyConstants.EMPLID) + hashObjectSize(CalculatedSalaryFoundationTracker.class, criteriaID, KFSPropertyConstants.EMPLID));
        ...
        queryID = new ReportQueryByCriteria(CalculatedSalaryFoundationTracker.class, columnList, criteriaID, true);
    ...
    private void setUpPositionNormalWorkMonths(Integer BaseYear) {
        ...
        while (positionRows.hasNext()) {
            ...
            positionNormalWorkMonths.put((String) positionRow[0], (Integer) ((BigDecimal) positionRow[1]).intValue());
        }
    }
    ...
    private void untouchedAppointmentFunding(PendingBudgetConstructionAppointmentFunding bcaf) {
        ...
        vacantCriteria.addEqualTo(KFSPropertyConstants.CSF_FUNDING_STATUS_CODE, BudgetConstructionConstants.VACANT_CSF_LINE);
        flagCriteria.addEqualTo(KFSPropertyConstants.CSF_FUNDING_STATUS_CODE, BudgetConstructionConstants.UNFUNDED_CSF_LINE);
        flagCriteria.addOrCriteria(vacantCriteria);
        // in addition, EMPLID is vacant
        vacantCriteria = new Criteria();
        vacantCriteria.addEqualTo(KFSPropertyConstants.EMPLID, BudgetConstructionConstants.VACANT_EMPLID);
    ...
    /*******************************************************************************************************************************
     * (10) this is a unit test routine
     * 
     * @@TODO: take this out
     ******************************************************************************************************************************/
    public void genesisUnitTest(Integer BaseYear) {
        /*
         * (1) check the SQL for appointment funding (worked 03/29/2007) p6spy=yes Criteria criteriaID = new Criteria();
         * criteriaID.addEqualTo(KFSPropertyConstants.UNIVERSITY_FISCAL_YEAR, BaseYear+1);
         * criteriaID.addEqualTo(KFSPropertyConstants.EMPLID,"0000001321"); QueryByCriteria queryID = new QueryByCriteria(
         * PendingBudgetConstructionAppointmentFunding.class, criteriaID); Iterator bcafResults =
         * getPersistenceBrokerTemplate().getIteratorByQuery(queryID); while (bcafResults.hasNext()) {
         * PendingBudgetConstructionAppointmentFunding bcaf = (PendingBudgetConstructionAppointmentFunding) bcafResults.next();
         * untouchedAppointmentFunding(bcaf); }
         */
        ...
        // }
        /*
         * (3) attempt to test the rounding mechanism (worked 4/4/2007) // build the new BC CSF objects in memory
         * setUpCSFOverrideKeys(BaseYear); setUpBCSFMap(BaseYear); setUpKeysNeedingRounding(BaseYear); readCSFOverride(BaseYear);
         * readCSF(BaseYear); CSFForBCSF = bCSF.size(); adjustCSFRounding(); CSFDiagnostics();
         */
        /*
         * (4) attempt to test appointment funding (worked 04/09/2007) createNewBCDocumentsFromGLCSF(2007,true,true);
    ...
    private void logAnExistingBCDocument() throws WorkflowException {
        ...
        KualiWorkflowDocument workflowDocument = workflowDocumentService.createWorkflowDocument(testDocumentID, GlobalVariables.getUserSession().getUniversalUser());
        ...
        workflowDocument = workflowDocumentService.createWorkflowDocument(testDocumentID, GlobalVariables.getUserSession().getUniversalUser());
    ...
    private void testANewBCDocument() throws WorkflowException {
        ...
        kualiDocumentHeader.setFinancialDocumentStatusCode(KFSConstants.INITIAL_KUALI_DOCUMENT_STATUS_CD);
        kualiDocumentHeader.setFinancialDocumentStatusCode(EdenConstants.ROUTE_HEADER_FINAL_CD);
        kualiDocumentHeader.setFinancialDocumentTotalAmount(KualiDecimal.ZERO);
        kualiDocumentHeader.setFinancialDocumentDescription(String.format("%s %d %s %s", BudgetConstructionConstants.BUDGET_CONSTRUCTION_DOCUMENT_DESCRIPTION, newBCHdr.getUniversityFiscalYear(), newBCHdr.getChartOfAccountsCode(), newBCHdr.getAccountNumber()));
         ...
         * newBCHdr.getDocumentHeader().getWorkflowDocument().complete(annotateDoc.toString()); // OK. This should have saved the
         * document. Now, we are going to try to set all the dates and codes finalizeBCDocument(newBCHdr);
         */
        saveBCDocumentInWorkflow(newBCHdr);
        // do this for a timing check
