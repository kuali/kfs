...
public interface CreditMemoService {
    ...
    /**
     * Makes call to dao to check for duplicate credit memos, and if one is found a message is returned. A duplicate error happens if there is
     * an existing credit memo with the same vendor number and credit memo number as the one which is being created, or same vendor number and
     * credit memo date.
     * 
    ...
    /**
     * Gets all credit memo documents associated with the given purchase order and has the status equal to one of the 
     * given status codes.
     * 
     * @param purchaseOrderIdentifier ID of the PO associated with the Credit Memo
