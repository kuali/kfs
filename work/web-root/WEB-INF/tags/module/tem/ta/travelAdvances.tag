...
<c:set var="documentAttributes"
	value="${DataDictionary.TravelAuthorizationDocument.attributes}" />
<c:set var="travelAdvanceAttributes"
	value="${DataDictionary.TravelAdvance.attributes}" />
<c:set var="advanceAttributes"
	value="${DataDictionary.AdvancePaymentReason.attributes}" />
<c:set var="docType"
	value="${KualiForm.document.dataDictionaryEntry.documentTypeName }" />
<c:set var="policyDisabled"
	value="${!(KualiForm.waitingOnTraveler || !fullEntryMode || KualiForm.showPolicy) }" />

<kul:tab tabTitle="Travel Advances"
	defaultOpen="${fn:length(KualiForm.document.travelAdvances) > 0}"
	tabErrorKey="${TemKeyConstants.TRVL_AUTH_TRVL_ADVANCE_ERRORS}">
	<div class="tab-container" align="left">
		...
		<c:if
			...
			<table cellpadding="0" cellspacing="0" class="datatable"
				...
				<tr>
					<td colspan="2" class="infoline"><c:if test="${fullEntryMode}">
							<div align=center>
								<html:image
		...
		<c:if test="${fn:length(KualiForm.document.travelAdvances) > 0}">
			<table>
				<logic:iterate indexId="ctr" name="KualiForm"
					property="document.travelAdvances" id="currentLine">
					...
					<c:set var="colSpan"
						value="${KualiForm.showPolicy?colSpan+1:colSpan}" />
					<c:set var="colSpan"
						value="${KualiForm.showPaymentMethods?colSpan+1:colSpan}" />
					<tr>
					...
					<tr>
						<td valign="top" colspan="3"><kul:htmlControlAttribute
								attributeEntry="${travelAdvanceAttributes.advancePaymentReasonCode}"
								property="document.travelAdvances[${ctr}].advancePaymentReasonCode"
								readOnly="${!fullEntryMode}" /></td>
						<td valign="top" colspan="${colSpan}"><kul:htmlControlAttribute
								attributeEntry="${travelAdvanceAttributes.additionalJustification}"
			...
			<c:if test="${KualiForm.showPolicy}">
    *By checking this box, the traveler agrees to the <a target="_BLANK"
					href="${KualiForm.policyURL}">travel advance policy</a> of the university.
    </c:if>
		</c:if>
	</div>
</kul:tab>
