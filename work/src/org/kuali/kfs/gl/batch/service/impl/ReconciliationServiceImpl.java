...
public class ReconciliationServiceImpl implements ReconciliationService {
     ...
     * representing each BO.
     * <br/><br/>
     * In the default implementation of {@link org.kuali.module.gl.service.ReconciliationParserService}, each 
     * {@link ColumnReconciliation} object may actually represent the sum of multiple fields across all origin entries
     * (i.e. ColumnReconciliation.getTokenizedFieldNames().length may be > 1).
     * <br/><br/>
     * Furthermore, the parser service returns database field names as the identifier.  This service requires the use
     * of java bean names, so this class is used to maintain a mapping between the DB names (in columnReconciliation.getTokenizedFieldNames())
     * and the java bean names (in javaAttributeNames).  These lists/arrays are the same size, and each element at the same position in both
     * lists are mapped to each other. 
     */
    ...
    public void reconcile(Iterator<OriginEntryFull> entries, ReconciliationBlock reconBlock, List<Message> errorMessages) {
        ...
        // - Misformatted origin entry line, which is not fatal (i.e. continue parsing the file and report further misformatted lines), but if it occurs, we don't want to do the final reconciliation step after this loop
    ...
    protected Message generateColumnSumErrorMessage(ColumnReconciliation column, KualiDecimal actualValue) {
        // TODO: if the kualiConfiguration service were to implement message params from ApplicationResources.properties, this would be ideal for that
     ...
     * @param javaAttributeNames the java attribute names corresponding to each field in columns.  (see {@link #resolveJavaAttributeNames(List)})
     ...
     * @return a list of {@link JavaAttributeAugmentedColumnReconciliation} (see class description) objects.  The returned list will have the same size as the parameter,
     * and each element in one list corresponds to the element at the same position in the other list 
     */
