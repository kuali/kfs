...
<%@ attribute name="invoiceAttributes" required="true" description="Attributes of Invoice according to the data dictionary"%>
...
<kul:tab tabTitle="Control Information" defaultOpen="true" tabErrorKey="${KFSConstants.CASH_CONTROL_DOCUMENT_ERRORS},document.hiddenFieldForErrors">
	...
	width: 50%;
							...
							<table width="100%" cellpadding="0" cellspacing="0" class="datatable">
								...
								<logic:iterate name="KualiForm" property="nonAppliedControlHoldings" id="nonApplied" indexId="idx">
									<tr>
										<td><kul:htmlControlAttribute attributeEntry="${nonAppliedHoldingAttributes.referenceFinancialDocumentNumber}"
												property="nonAppliedControlHoldings[${idx}].referenceFinancialDocumentNumber" readOnly="true" /> <!--<bean:write name="nonApplied" property="referenceFinancialDocumentNumber" />-->
										</td>
										<c:if test="${readOnly ne true}">
											<td style="text-align: right;"><kul:htmlControlAttribute attributeEntry="${nonAppliedHoldingAttributes.availableUnappliedAmount}"
													property="nonAppliedControlHoldings[${idx}].availableUnappliedAmount" readOnly="true" /> <!--<bean:write name="nonApplied" property="availableUnappliedAmount" />-->
											</td>
										</c:if>
								...
								<tr>
									...
									<td style="text-align: right;"><kul:htmlControlAttribute attributeEntry="${totalApplied}" property="totalApplied" readOnly="true" /></td>
								</tr>
							</table>
						</div>
...
#ctrl-info th,#ctrl-info td {
	width: 50%;
					...
					<table id='ctrl-info' width="100%" cellpadding="0" cellspacing="0" class="datatable">
						...
						<tr>
							<th>Customer</th>
							<td><kul:htmlControlAttribute attributeEntry="${cashControlDetailAttributes.customerNumber}"
									property="document.cashControlDetail.customerNumber" readOnly="true" /></td>
						</tr>
						<tr>
							<th>Control Total</th>
							<td><kul:htmlControlAttribute attributeEntry="${cashControlDetailAttributes.financialDocumentLineAmount}"
									property="document.cashControlDetail.financialDocumentLineAmount" readOnly="true" /></td>
						</tr>
						<tr>
							<th>Open Amount</th>
							<td><c:choose>
									<c:when test="${empty KualiForm.editingMode['errorCorrection']}">
										<kul:htmlControlAttribute attributeEntry="${unallocatedBalance}" property="unallocatedBalance" readOnly="true" />
									</c:when>
									<c:otherwise>
								          0.00
							          </c:otherwise>
