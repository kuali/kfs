...
public interface CashManagementService {
    ...
    /**
     * Apply a cashiering transaction to a cash management document. This means: 0. check rules??? 1. Updating the cash drawer with
     * any incoming currency and coin 2. Moving any checks from the transaction to the CM document 3. Checking if any items in
     * process were closed; if so, saving that info 4. Saving currency and coin records 5. Saving any new item in process 6. saving
     * any checks
     * 
     ...
     * 
     * @param cmDoc the cash management document which is associated with the workgroup that the closed items in process would have
    ...
    /**
     * Generates the master currency detail, which sounds bad, but which is really just okay. A master currency detail is the
     * composite effect of all the transactions of a cash management document on a cash drawer
     * 
     * @param cmDoc the cash management document to generate the master record for
     * @return the master record, master in the sense of "Platonic ideal" from which all else is a copy
     */
