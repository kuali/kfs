...
<xsl:stylesheet version="1.0"
  ...
  <!--                           VERSION HISTORY
    ...
    - 01/2003: Fixes in comments. Also experimented further with merging the
               ...
               Unfortunately no working solution was found in a timely manner.
    - 02/2003: Two bug fixes:
               - column span was incorrect in med level non-personnel section.
               - for multiple tasks/one period the page breaks were broken.
               Two enhancements:
               - Personnel is now sorted after the CREATE_TIMESTAMP which is
                 passed by budget
               - Personnel is now also collapsed after CREATE_TIMESTAMP. This
                 more accurately models what Budget is doing.
    - 04/2004: Bug fix:
               - Personnel collapsing for task / period summary had a bug
                 for tasks beyond the first. Some personnel would not
                 show. Fixed using Multiple Level Muenchian Method Grouping:
  ...
  <!--            NOTES TO IMPLEMENTATION OF THIS XSLT-FO FILE
  Due to time constraint, new development tool (xslt-fo), and some features
  not implemented yet in apache fop, there are several things about this xslt
  file that can be improved.
  - This file has three gigantic and almost identical templates.  The only
    difference in the three are the query strings. They refer to retrieving
    (call is made from "Root Template"):
    - one task, one period: ./
    - one task, all periods (of that task): ../TASK_PERIOD[@TASK_NUMBER = $taskNumber]
    - all tasks, all periods: ../TASK_PERIOD[*]
    If the three templates are merged, the code in this file can be almost
    slashed by 1/3. I attempted to merge the three templates (and thus
    eliminate the gigantic code replication) with several approaches outlined
    below:
    A) Parameters passed to templates.  That also meant evaluating variable
    strings into node sets with "xalan:nodeset" but unfortunately that never
    worked (explaining this is difficult and I'd rather have other developers
    start on a fresh slate then confusing them with my confusion:).
    <xsl:variable name="PATH_ByTaskByPeriod">/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD[(@TASK_NUMBER = $taskNumber) and (@PERIOD_NUMBER = $periodNumber)]/PERSONNEL</xsl:variable>
    <xsl:variable name="PATH_ByTaskBySummary">/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL</xsl:variable>
    <xsl:variable name="PATH_BySummaryBySummary">/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD[*]/PERSONNEL[*]</xsl:variable>
    B) Axis accessors such as "parent", "preceding", etc. Please read Jeni
    Tennison notes on this issue. In particular this:
    "I guess so :) *All* XPath axes work on the node tree, *not* on the
     ...
     in the node tree."
    This problem send me spinning since then I'd need a way to keep track of my
    current node (what task, what period?) and that would bring me back to
    using solution.
    C) David Schalk introduced me to the ideas of xsl:key's. This seemed
    promising and I haven't exhausted options in this area. Due to time
    constraints I had to stop. In essence the problem was the regular keys
    only index by one parameter (see "use"). There are exceptions but none of
    them got me further. The best I got was the below code, unfortunately the
    intersection turned out to be an empty set (even though I believe it
    shouldn't be):
    <xsl:key name="KEY-TASK_PERIOD-byTask" match="/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD" use="@TASK_NUMBER"/>
    <xsl:key name="KEY-TASK_PERIOD-byPeriod" match="/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD" use="@PERIOD_NUMBER"/>
    <xsl:variable name="taskSet" select="key('KEY-TASK_PERIOD-byTask',$whatTaskPath)"/>
    <xsl:variable name="periodSet" select="key('KEY-TASK_PERIOD-byPeriod',$whatPeriodPath)"/>
    <xsl:variable name="taskIntersectionPeriodSet" select="$taskSet[count(. | $periodSet) = count($periodSet)]"/>
    To be used like this:
    <xsl:for-each select="$taskIntersectionPeriodSet/PERSONNEL/PERSON"> [...]
    Most of those ideas were taken from Michael Kay's XSLT 2nd Edition book
    (published by wrox, ISBN 1-861005-06-7).
  - xsl:key should be used. Currently there are a lot of repetitive and big
    query statements that could be limited with using keys.
  - format-number(...) is sprinkled all over the file. I was wondering if some
    global parameter can be set but wasn't successful to finding how this could
    possibly be done.
  - lowercase to uppercase conversion can be slightly simplified. See here:
    http://www.dpawson.co.uk/xsl/sect2/N6461.html#d195e1117
    That example solves a different problem but makes use of
    case-identification in a more elegant way.
  - A lot of "for-each" loops were used while templates would have probably
    been nicer (more structured).
  - On the personnel roll up, notice that logically NAME doesn't add any
    information content to what TIMESTAMP already gives, so checking for
    NAME is obsolete. I didn't remove it because it was too close to the
    release.

    REFERENCES
  - This method is used for sorting non personnel detail
    (non-alphabetically):
    http://www.dpawson.co.uk/xsl/sect2/N6461.html#d195e526
    An issue that was difficult to resolve. It is probably in an acceptable
    state now, but I wanted to take the time to reference that.
  - Another very useful pointer:
    http://www.jenitennison.com/
    I took a lot of resources from that link. In particular it has information
    about Grouping and axis accessors worthwhile to look over.
  -->
  ...
  <!-- Following key is used to retrieve people of a certain task with
       a certain name, appointment_type, and timestamp. Used to
       collapse duplicates in personnel summaries. -->
  <xsl:key name="PEOPLE"
           match="PERSON"
           use="concat(../../@TASK_NUMBER, ' ',
                       NAME, ' ',
                       APPOINTMENT/@APPOINTMENT_TYPE, ' ',
                       @SEQUENCE_NUMBER)" />
  
  <!-- 4/18/2005 dterret: This key was added as part of the fix to the problem 
			 of the Unrecovered Indirect Cost not showing up in the Institution
			 Total Cost Share and Total Cost for each Task and Period. It is
			 used to retrieve the Unrecovered Indirect in each Task_Period.-->
  <xsl:key name="UNRECOVERED"
	...
	<xsl:key name="taskPeriodPersonKey"
        			...
        			<fo:table-row>
        				...
        				<fo:table-cell>
        					<fo:block font-family="Times" font-size="6pt" text-align="right">
        						Tracking Number: B <xsl:value-of select="/PROPOSAL/BUDGET/@BUDGET_NUMBER"/>
        					</fo:block>
        				</fo:table-cell>
        			</fo:table-row>
        		</fo:table-body>
  ...
  <xsl:template match="/PROPOSAL/BUDGET/TASK_PERIODS/TASK_PERIOD">
    <!-- We always need one Task for one Period (template won't fire if we
         are out of those). -->
    <xsl:call-template name="taskPeriodPage"/>
    ...
    <!-- If this was the last PERIOD in a TASK, we need a period summary page
         except if there only was one period. -->
    <xsl:if test="not(following::TASK_PERIOD[@TASK_NUMBER = current()/@TASK_NUMBER]) and (count(/PROPOSAL/BUDGET/PERIODS/PERIOD[*]) > 1)">
      ...
      <xsl:if test="not(position() = last()) or ((position() = last()) and (count(/PROPOSAL/BUDGET/TASKS/TASK[*]) > 1))">
        <fo:block break-before="page"/>
      </xsl:if>
    </xsl:if>
    ...
    <xsl:if test="(position() = last()) and (count(/PROPOSAL/BUDGET/TASKS/TASK[*]) > 1)">
  ...
  <!-- *******************      by Task, by Period      ******************* -->
  <!-- ******************* **************************** ******************* -->
  <xsl:template name="taskPeriodPage">
    <!-- ************************* VARIABLES ************************* -->
    ...
    <fo:table-header>
      ...
      <fo:table-row>
        <fo:table-cell number-columns-spanned="7">
          ...
          <!-- Only put the task if there is more then one -->
          <xsl:if test="/PROPOSAL/BUDGET/TASKS/TASK[2]">
            <fo:block>
  ...
  <xsl:template name="summaryPage">
    ...
    <fo:table-body>
      ...
      <xsl:if test="$levelOfDetail != 'low'">
        ...
        <xsl:for-each select="../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL/PERSON[generate-id(.) = generate-id(key('PEOPLE',
                                    concat(../../@TASK_NUMBER, ' ',
                                           NAME, ' ',
                                           APPOINTMENT/@APPOINTMENT_TYPE, ' ',
                                           @SEQUENCE_NUMBER)))]">
        ...
        <xsl:sort data-type="text" lang="en" case-order="upper-first" order="ascending" select="@SEQUENCE_NUMBER"/>
            ...
            <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
              <fo:block xsl:use-attribute-sets='defaultText'>
                <xsl:value-of select="format-number(sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL/PERSON[(NAME = current()/NAME) and (APPOINTMENT/@APPOINTMENT_TYPE = current()/APPOINTMENT/@APPOINTMENT_TYPE and @SEQUENCE_NUMBER = current()/@SEQUENCE_NUMBER)]/@AGENCY_AMOUNT_SALARY), '###,###')"/>
              </fo:block>
            </fo:table-cell>
            <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="center">
            ...
            <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
              <fo:block xsl:use-attribute-sets='defaultText'>
                <xsl:value-of select="format-number(sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL/PERSON[(NAME = current()/NAME) and (APPOINTMENT/@APPOINTMENT_TYPE = current()/APPOINTMENT/@APPOINTMENT_TYPE and @SEQUENCE_NUMBER = current()/@SEQUENCE_NUMBER)]/@INSTITUTION_AMOUNT_SALARY), '###,###')"/>
              </fo:block>
            </fo:table-cell>
            <!-- Next cell: Notice there is no 3rd party cost share for Personnel -->
            <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
            ...
            <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
              <fo:block xsl:use-attribute-sets='defaultText'>
                <xsl:value-of select="format-number(sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL/PERSON[(NAME = current()/NAME) and (APPOINTMENT/@APPOINTMENT_TYPE = current()/APPOINTMENT/@APPOINTMENT_TYPE and @SEQUENCE_NUMBER = current()/@SEQUENCE_NUMBER)]/@AGENCY_AMOUNT_SALARY) + sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL/PERSON[(NAME = current()/NAME) and (APPOINTMENT/@APPOINTMENT_TYPE = current()/APPOINTMENT/@APPOINTMENT_TYPE and @SEQUENCE_NUMBER = current()/@SEQUENCE_NUMBER)]/@INSTITUTION_AMOUNT_SALARY), '###,###')"/>
              </fo:block>
            </fo:table-cell>
          </fo:table-row>
        </xsl:for-each>
      ...
      <fo:table-row>
        ...
        <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL[*]/PERSON/@AGENCY_AMOUNT_SALARY), '###,###')"/>
          </fo:block>
        </fo:table-cell>
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder'/>
        <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL[*]/PERSON/@INSTITUTION_AMOUNT_SALARY), '###,###')"/>
          </fo:block>
        </fo:table-cell>
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">0</fo:block>
        ...
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL[*]/PERSON/@AGENCY_AMOUNT_SALARY) + sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL[*]/PERSON/@INSTITUTION_AMOUNT_SALARY), '###,###')"/>
          </fo:block>
        </fo:table-cell>
      </fo:table-row>
      ...
      <xsl:if test="$levelOfDetail != 'low'">
        ...
        <xsl:for-each select="../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL/PERSON[generate-id(.) = generate-id(key('PEOPLE',
          ...
          <fo:table-row>
            ...
            <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="center">
              <fo:block xsl:use-attribute-sets='defaultText'>
                <xsl:if test="count(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL/PERSON[(NAME = current()/NAME) and (APPOINTMENT/@APPOINTMENT_TYPE = current()/APPOINTMENT/@APPOINTMENT_TYPE and @SEQUENCE_NUMBER = current()/@SEQUENCE_NUMBER)]) = 1">
                  <xsl:value-of select="./@AGENCY_FRINGE_BENEFIT_RATE"/>
                </xsl:if>
              </fo:block>
            </fo:table-cell>
            <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
              <fo:block xsl:use-attribute-sets='defaultText'>
                <xsl:value-of select="format-number(sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL/PERSON[(NAME = current()/NAME) and (APPOINTMENT/@APPOINTMENT_TYPE = current()/APPOINTMENT/@APPOINTMENT_TYPE and @SEQUENCE_NUMBER = current()/@SEQUENCE_NUMBER)]/@AGENCY_FRINGE_BENEFIT_AMOUNT), '###,###')"/>
              </fo:block>
            </fo:table-cell>
            <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="center">
              <fo:block xsl:use-attribute-sets='defaultText'>
            ...
            <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
              <fo:block xsl:use-attribute-sets='defaultText'>
                <xsl:value-of select="format-number(sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL/PERSON[(NAME = current()/NAME) and (APPOINTMENT/@APPOINTMENT_TYPE = current()/APPOINTMENT/@APPOINTMENT_TYPE and @SEQUENCE_NUMBER = current()/@SEQUENCE_NUMBER)]/@INSTITUTION_FRINGE_BENEFIT_AMOUNT), '###,###')"/>
              </fo:block>
            </fo:table-cell>
            <!-- Next cell: Notice there is no 3rd party cost share for Personnel -->
            <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
            ...
            <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
              <fo:block xsl:use-attribute-sets='defaultText'>
                <xsl:value-of select="format-number(sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL/PERSON[(NAME = current()/NAME) and (APPOINTMENT/@APPOINTMENT_TYPE = current()/APPOINTMENT/@APPOINTMENT_TYPE and @SEQUENCE_NUMBER = current()/@SEQUENCE_NUMBER)]/@AGENCY_FRINGE_BENEFIT_AMOUNT) + sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL/PERSON[(NAME = current()/NAME) and (APPOINTMENT/@APPOINTMENT_TYPE = current()/APPOINTMENT/@APPOINTMENT_TYPE and @SEQUENCE_NUMBER = current()/@SEQUENCE_NUMBER)]/@INSTITUTION_FRINGE_BENEFIT_AMOUNT), '###,###')"/>
              </fo:block>
            </fo:table-cell>
          </fo:table-row>
        </xsl:for-each>
      ...
      <fo:table-row>
        ...
        <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL[*]/PERSON/@AGENCY_FRINGE_BENEFIT_AMOUNT), '###,###')"/>
          </fo:block>
        </fo:table-cell>
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder'/>
        <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL[*]/PERSON/@INSTITUTION_FRINGE_BENEFIT_AMOUNT), '###,###')"/>
          </fo:block>
        </fo:table-cell>
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">0</fo:block>
        ...
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL[*]/PERSON/@AGENCY_FRINGE_BENEFIT_AMOUNT) + sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL[*]/PERSON/@INSTITUTION_FRINGE_BENEFIT_AMOUNT), '###,###')"/>
          </fo:block>
        </fo:table-cell>
      </fo:table-row>
      ...
      <fo:table-row>
        ...
        <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL[*]/@TOTAL_AGENCY_PERSONNEL), '###,###')"/>
          </fo:block>
        </fo:table-cell>
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder'/>
        <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL[*]/@TOTAL_INSTITUTION_PERSONNEL), '###,###')"/>
          </fo:block>
        </fo:table-cell>
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">0</fo:block>
        ...
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL[*]/@TOTAL_AGENCY_PERSONNEL) + sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/PERSONNEL[*]/@TOTAL_INSTITUTION_PERSONNEL), '###,###')"/>
          </fo:block>
        </fo:table-cell>
      </fo:table-row>
      ...
      <!-- List the non personnel items -->
      <xsl:variable name="npItems" select="../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/NON_PERSONNEL/NON_PERSONNEL_ITEM"/>
      <xsl:for-each select="$npItems">
        <!-- Following line is arbitrary sorting (see nonp:category at top
             of file -->
        <xsl:sort select="xalan:nodeset($CATEGORYMAP)/nonp/category[@name=current()/CATEGORY]/@value" data-type="number" />
        ...
        <xsl:if test="generate-id(.)=
          ...
          <xsl:choose>
            <xsl:when test="$levelOfDetail = 'high'">
              ...
              <xsl:apply-templates select="../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/NON_PERSONNEL/NON_PERSONNEL_ITEM[CATEGORY = current()/CATEGORY]">
                <xsl:sort data-type="text" select="SUB_CATEGORY"/>
              </xsl:apply-templates>
            </xsl:when>
            <!-- Display category summary if med level of detail -->
              ...
              <xsl:for-each select="$npItems[CATEGORY=current()/CATEGORY]">
                ...
                <xsl:if test="generate-id(.)=
                  ...
                  <fo:table-row>
                    ...
                    <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
                      <fo:block xsl:use-attribute-sets='defaultText'>
                        <xsl:value-of select="format-number(sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/NON_PERSONNEL/NON_PERSONNEL_ITEM[SUB_CATEGORY = current()/SUB_CATEGORY]/AGENCY_REQUEST_AMOUNT), '###,###')"/>
                      </fo:block>
                    </fo:table-cell>
                    <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder'/>
                    <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
                      <fo:block xsl:use-attribute-sets='defaultText'>
                        <xsl:value-of select="format-number(sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/NON_PERSONNEL/NON_PERSONNEL_ITEM[SUB_CATEGORY = current()/SUB_CATEGORY]/INSTITUTION_COST_SHARE_AMOUNT), '###,###')"/>
                      </fo:block>
                    </fo:table-cell>
                    <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
                      <fo:block xsl:use-attribute-sets='defaultText'>
                        <xsl:value-of select="format-number(sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/NON_PERSONNEL/NON_PERSONNEL_ITEM[SUB_CATEGORY = current()/SUB_CATEGORY]/THIRD_PARTY_COST_SHARE_AMOUNT), '###,###')"/>
                      </fo:block>
                    </fo:table-cell>
                    <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
                      <fo:block xsl:use-attribute-sets='defaultText'>
                        <xsl:value-of select="format-number(sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/NON_PERSONNEL/NON_PERSONNEL_ITEM[SUB_CATEGORY = current()/SUB_CATEGORY]/AGENCY_REQUEST_AMOUNT) + sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/NON_PERSONNEL/NON_PERSONNEL_ITEM[SUB_CATEGORY = current()/SUB_CATEGORY]/INSTITUTION_COST_SHARE_AMOUNT) + sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/NON_PERSONNEL/NON_PERSONNEL_ITEM[SUB_CATEGORY = current()/SUB_CATEGORY]/THIRD_PARTY_COST_SHARE_AMOUNT), '###,###')"/>
                      </fo:block>
                    </fo:table-cell>
                  </fo:table-row>
                </xsl:if>
          ...
          <fo:table-row>
            ...
            <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
              <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
                <xsl:value-of select="format-number(sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/NON_PERSONNEL/NON_PERSONNEL_ITEM[CATEGORY = current()/CATEGORY]/AGENCY_REQUEST_AMOUNT), '###,###')"/>
              </fo:block>
            </fo:table-cell>
            <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder'/>
            <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
              <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
                <xsl:value-of select="format-number(sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/NON_PERSONNEL/NON_PERSONNEL_ITEM[CATEGORY = current()/CATEGORY]/INSTITUTION_COST_SHARE_AMOUNT), '###,###')"/>
              </fo:block>
            </fo:table-cell>
            <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
              <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
                <xsl:value-of select="format-number(sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/NON_PERSONNEL/NON_PERSONNEL_ITEM[CATEGORY = current()/CATEGORY]/THIRD_PARTY_COST_SHARE_AMOUNT), '###,###')"/>
              </fo:block>
            </fo:table-cell>
            <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
              <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
                <xsl:value-of select="format-number(sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/NON_PERSONNEL/NON_PERSONNEL_ITEM[CATEGORY = current()/CATEGORY]/AGENCY_REQUEST_AMOUNT) + sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/NON_PERSONNEL/NON_PERSONNEL_ITEM[CATEGORY = current()/CATEGORY]/INSTITUTION_COST_SHARE_AMOUNT) + sum(../../../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/NON_PERSONNEL/NON_PERSONNEL_ITEM[CATEGORY = current()/CATEGORY]/THIRD_PARTY_COST_SHARE_AMOUNT), '###,###')"/>
              </fo:block>
            </fo:table-cell>
          </fo:table-row>
      ...
      <fo:table-row keep-with-next="always">
        ...
        <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_AGENCY_REQUEST_DIRECT_COST), '###,###')"/>
          </fo:block>
        </fo:table-cell>
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder'/>
        <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_INSTITUTION_DIRECT_COST), '###,###')"/>
          </fo:block>
        </fo:table-cell>
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_THIRD_PARTY_DIRECT_COST), '###,###')"/>
          </fo:block>
        </fo:table-cell>
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_AGENCY_REQUEST_DIRECT_COST) + sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_INSTITUTION_DIRECT_COST) + sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_THIRD_PARTY_DIRECT_COST), '###,###')"/>
          </fo:block>
        </fo:table-cell>
      </fo:table-row>
      ...
      <fo:table-row keep-with-next="always">
        ...
        <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-style="italic">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_AGENCY_REQUEST_INDIRECT_COST), '###,###')"/>
          </fo:block>
        </fo:table-cell>
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder'/>
        <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-style="italic">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_INSTITUTION_INDIRECT_COST), '###,###')"/>
          </fo:block>
        </fo:table-cell>
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-style="italic">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_THIRD_PARTY_INDIRECT_COST), '###,###')"/>
          </fo:block>
        </fo:table-cell>
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-style="italic">
            <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_AGENCY_REQUEST_INDIRECT_COST) + sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_INSTITUTION_INDIRECT_COST) + sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_THIRD_PARTY_INDIRECT_COST), '###,###')"/>
          </fo:block>
        </fo:table-cell>
      </fo:table-row>
      ...
      <fo:table-row keep-with-next="always">
        ...
        <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold" border-bottom-style="solid" border-bottom-width="thin" border-top-style="solid" border-top-width="thin">
            $ <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_AGENCY_COST), '###,###')"/>
          </fo:block>
        </fo:table-cell>
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder'/>
        ...
        <fo:table-cell xsl:use-attribute-sets='tableCell' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold" border-bottom-style="solid" border-bottom-width="thin" border-top-style="solid" border-top-width="thin">
            $ <xsl:value-of select="format-number(sum(../TASK_PERIOD
            	[@TASK_NUMBER = $taskNumber]/@TOTAL_INSTITUTION_COST), '###,###')"/>
          </fo:block>
        </fo:table-cell>
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold" border-bottom-style="solid" border-bottom-width="thin" border-top-style="solid" border-top-width="thin">
            $ <xsl:value-of select="format-number(sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_THIRD_PARTY_COST), '###,###')"/>
          </fo:block>
        </fo:table-cell>
        <!-- The following was changed as part of a temporary fix. DT 4/18/2005 -->
      	<!-- 5/10/2005 dterret: The temporary change was backed out here. -->
        <fo:table-cell xsl:use-attribute-sets='tableCellLeftBorder' text-align="right">
          <fo:block xsl:use-attribute-sets='defaultText' font-weight="bold" border-bottom-style="solid" border-bottom-width="thin" border-top-style="solid" border-top-width="thin">
            $ <xsl:value-of select="format-number(
            	sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_AGENCY_COST) + 
              sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_INSTITUTION_COST) +
              sum(../TASK_PERIOD[@TASK_NUMBER = $taskNumber]/@TOTAL_THIRD_PARTY_COST), '###,###')"/>
          </fo:block>
        </fo:table-cell>
      </fo:table-row>
