...
public class AccountsReceivableUtils {
    ...
    public static boolean canCorrectDetail(CashControlDetail cashControlDetail) {
        ...
        try {
            PaymentApplicationDocument paymentApplicationDocument = (PaymentApplicationDocument) KNSServiceLocator.getDocumentService().getByDocumentHeaderId(paymentApplicationDocumentNumber);
            ...
            if (workflowDocument.stateIsFinal() && !paymentApplicationDocument.isPaymentApplicationCorrection() && StringUtils.isBlank(paymentApplicationDocument.getDocumentHeader().getCorrectedByDocumentId()) && hasDataValuesToCorrect(paymentApplicationDocument)) {
                return true;
            }
