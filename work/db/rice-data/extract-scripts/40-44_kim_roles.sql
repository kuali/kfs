-- KFS Roles
SELECT ROLE_ID, 'KFS'||ROLE_ID AS OBJ_ID, ROLE_NM, NMSPC_CD, DESC_TXT, KIM_TYP_ID, ACTV_IND 
    FROM KRIM_ROLE_T
...
SELECT 'KFS'||ROLE_MBR_ID AS ROLE_MBR_ID, 'KFS'||ROLE_MBR_ID AS OBJ_ID, ROLE_ID, CASE WHEN mbr_typ_cd = 'P' THEN 'KFS'||MBR_ID ELSE mbr_id END AS MBR_ID, MBR_TYP_CD
...
UNION ALL -- KFS roles (as members) assigned to rice roles
    SELECT 'KFS'||ROLE_MBR_ID AS ROLE_MBR_ID, 'KFS'||ROLE_MBR_ID AS OBJ_ID, ROLE_ID, MBR_ID, MBR_TYP_CD
...
-- Delegations
SELECT 'KFS'||DLGN_ID AS DLGN_ID, 1 AS VER_NBR, 'KFS'||DLGN_ID AS OBJ_ID, ROLE_ID, ACTV_IND, KIM_TYP_ID, DLGN_TYP_CD 
    FROM KRIM_DLGN_T
    WHERE role_id IN (
        SELECT ROLE_ID
            FROM KRIM_ROLE_T
...
SELECT 'KFS'||ATTR_DATA_ID AS ATTR_DATA_ID, 'KFS'||ATTR_DATA_ID AS OBJ_ID, 1 AS VER_NBR, 'KFS'||ROLE_MBR_ID AS ROLE_MBR_ID, KIM_TYP_ID, KIM_ATTR_DEFN_ID, ATTR_VAL 
...
-- Delegation Members
SELECT 'KFS'||DLGN_MBR_ID AS DLGN_MBR_ID, 'KFS'||DLGN_MBR_ID AS OBJ_ID, 'KFS'||DLGN_ID AS DLGN_ID, CASE WHEN mbr_typ_cd = 'P' THEN 'KFS'||MBR_ID ELSE mbr_id END AS MBR_ID, MBR_TYP_CD, 'KFS'||ROLE_MBR_ID AS ROLE_MBR_ID 
...
SELECT 'KFS'||ATTR_DATA_ID AS ATTR_DATA_ID, 'KFS'||ATTR_DATA_ID AS OBJ_ID, 'KFS'||DLGN_MBR_ID AS DLGN_MBR_ID, KIM_TYP_ID, KIM_ATTR_DEFN_ID, ATTR_VAL 
