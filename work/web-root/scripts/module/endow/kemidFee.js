 ...
 */
 ...
 function loadFeeStartDate(feeMethodCodeFieldName){
	...
	if (feeMethodCode =='') {
		...
		var dwrReply = {
			...
			if ( data != null ) {
				...
				setRecipientValue( feeStartDateFieldName, wrapError( "failed to display fee start date" ), true );			
			} },
			errorHandler:function( errorMessage ) { 
				setRecipientValue( feeStartDateFieldName, wrapError( "failed to display fee start date" ), true );
			}
		};
		FeeMethodService.getFeeMethodNextProcessDateForAjax( feeMethodCode, dwrReply );
	}
}
