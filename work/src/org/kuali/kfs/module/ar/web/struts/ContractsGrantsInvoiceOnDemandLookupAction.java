...
/**
 * Action class for Contracts Grants Invoice On-Demand Lookup.
 */
public class ContractsGrantsInvoiceOnDemandLookupAction extends KualiMultipleValueLookupAction {

    private static final org.apache.log4j.Logger LOG = org.apache.log4j.Logger.getLogger(ContractsGrantsInvoiceOnDemandLookupAction.class);
    ...
    protected List<ResultRow> selectAll(MultipleValueLookupForm multipleValueLookupForm, int maxRowsPerPage) {
        ...
        for (ResultRow row : resultTable) {
            ...
            if (row instanceof ContractsGrantsInvoiceOnDemandResultRow) {
                for (ResultRow subResultRow : ((ContractsGrantsInvoiceOnDemandResultRow) row).getSubResultRows()) {
    ...
    public ActionForward prepareToReturnSelectedResults(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (success) {
            ...
            parameters.put(KNSConstants.DISPATCH_REQUEST_PARAMETER, "viewSummary");

            String contractsGrantsInvoiceOnDemandSummaryUrl = UrlFactory.parameterizeUrl("arContractsGrantsInvoiceOnDemandSummary.do", parameters);
            return new ActionForward(contractsGrantsInvoiceOnDemandSummaryUrl, true);
