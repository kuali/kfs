...
public class RoutingFormAuditRule {
    ...
    private static boolean processRoutingFormProjectDetailsAuditChecks(RoutingFormDocument routingFormDocument) {
        ...
        if (!valid) {
            auditErrors.add(new AuditError("document.projectDetails.otherProjectDetailsQuestions", CGKeyConstants.AUDIT_OTHER_PROJECT_DETAILS_NOT_SELECTED, "projectdetails.anchor1"));
    ...
    private static boolean processRoutingFormMainPageAgencyDeliveryAuditChecks(List<AuditError> auditErrors, RoutingFormDocument routingFormDocument) {
        ...
        if (routingFormDocument.isRoutingFormAgencyToBeNamedIndicator() || ObjectUtils.isNull(routingFormDocument.getRoutingFormAgency().getAgencyNumber())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormAgency.agencyNumber", CGKeyConstants.AUDIT_MAIN_PAGE_AGENCY_REQUIRED, "mainpage.anchor1"));
        }

        if (ObjectUtils.isNull(routingFormDocument.getRoutingFormAgency().getRoutingFormDueDateTypeCode())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormAgency.routingFormDueDateTypeCode", CGKeyConstants.AUDIT_MAIN_PAGE_DUE_DATE_TYPE_REQUIRED, "mainpage.anchor1"));
        }

        if (ObjectUtils.isNull(routingFormDocument.getRoutingFormAgency().getRoutingFormRequiredCopyText())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormAgency.routingFormRequiredCopyText", CGKeyConstants.AUDIT_MAIN_PAGE_COPIES_REQUIRED, "mainpage.anchor1"));
        }

        if (ObjectUtils.isNull(routingFormDocument.getRoutingFormAgency().getAgencyAddressDescription())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormAgency.agencyAddressDescription", CGKeyConstants.AUDIT_MAIN_PAGE_ADDRESS_REQUIRED, "mainpage.anchor1"));
        }
    ...
    private static boolean processRoutingFormMainPagePersonnelUnitsAuditChecks(List<AuditError> auditErrors, RoutingFormDocument routingFormDocument) {
        final String PERSON_ROLE_CODE_OTHER = SpringContext.getBean(ParameterService.class).getParameterValue(RoutingFormDocument.class, CGConstants.PERSON_ROLE_CODE_OTHER);
        ...
        for (RoutingFormPersonnel person : routingFormDocument.getRoutingFormPersonnel()) {
            ...
            if (ObjectUtils.isNull(person.getPersonRoleCode())) {
                valid = false;
                auditErrors.add(new AuditError("document.routingFormPersonnel[" + i + "].personRoleCode", CGKeyConstants.AUDIT_MAIN_PAGE_PERSON_ROLE_CODE_REQUIRED, "mainpage.anchor2"));
            }
            else if (PERSON_ROLE_CODE_OTHER.equals(person.getPersonRoleCode()) && ObjectUtils.isNull(person.getPersonRoleText())) {
                valid = false;
                auditErrors.add(new AuditError("document.routingFormPersonnel[" + i + "].personRoleText", CGKeyConstants.AUDIT_MAIN_PAGE_PERSON_ROLE_TEXT_REQUIRED, "mainpage.anchor2"));
            }

            if (ObjectUtils.isNull(person.getPersonFinancialAidPercent())) {
                valid = false;
                auditErrors.add(new AuditError("document.routingFormPersonnel[" + i + "].personFinancialAidPercent", CGKeyConstants.AUDIT_MAIN_PAGE_PERSON_FA_REQUIRED, "mainpage.anchor2"));
            }

            if (ObjectUtils.isNull(person.getPersonCreditPercent())) {
                valid = false;
                auditErrors.add(new AuditError("document.routingFormPersonnel[" + i + "].personCreditPercent", CGKeyConstants.AUDIT_MAIN_PAGE_PERSON_CREDIT_REQUIRED, "mainpage.anchor2"));
            }
        ...
        for (RoutingFormOrganizationCreditPercent org : routingFormDocument.getRoutingFormOrganizationCreditPercents()) {
            if (ObjectUtils.isNull(org.getOrganizationFinancialAidPercent())) {
                valid = false;
                auditErrors.add(new AuditError("document.routingFormOrganizationCreditPercents[" + i + "].personFinancialAidPercent", CGKeyConstants.AUDIT_MAIN_PAGE_ORG_FA_REQUIRED, "mainpage.anchor2"));
            }

            if (ObjectUtils.isNull(org.getOrganizationCreditPercent())) {
                valid = false;
                auditErrors.add(new AuditError("document.routingFormOrganizationCreditPercents[" + i + "].personCreditPercent", CGKeyConstants.AUDIT_MAIN_PAGE_ORG_CREDIT_REQUIRED, "mainpage.anchor2"));
            }
        ...
        if (!new KualiInteger(100).equals(routingFormDocument.getTotalFinancialAidPercent())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormPersonnel.personFinancialAidPercent", CGKeyConstants.AUDIT_MAIN_PAGE_TOTAL_FA_PERCENT_NOT_100, "mainpage.anchor2"));
        }

        if (!new KualiInteger(100).equals(routingFormDocument.getTotalCreditPercent())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormPersonnel.personCreditPercent", CGKeyConstants.AUDIT_MAIN_PAGE_TOTAL_CREDIT_PERCENT_NOT_100, "mainpage.anchor2"));
        }
    ...
    private static boolean processRoutingFormMainPageSubmissionDetailsAuditChecks(List<AuditError> auditErrors, RoutingFormDocument routingFormDocument) {
        ...
        final String SUBMISSION_TYPE_CHANGE = parameterService.getParameterValue(RoutingFormDocument.class, CGConstants.SUBMISSION_TYPE_CHANGE);
        final String PROJECT_TYPE_NEW = parameterService.getParameterValue(RoutingFormDocument.class, CGConstants.PROJECT_TYPE_NEW);
        final String PROJECT_TYPE_TIME_EXTENTION = parameterService.getParameterValue(RoutingFormDocument.class, CGConstants.PROJECT_TYPE_TIME_EXTENTION);
        final String PROJECT_TYPE_BUDGET_REVISION_ACTIVE = parameterService.getParameterValue(RoutingFormDocument.class, CGConstants.PROJECT_TYPE_BUDGET_REVISION_ACTIVE);
        final String PROJECT_TYPE_BUDGET_REVISION_PENDING = parameterService.getParameterValue(RoutingFormDocument.class, CGConstants.PROJECT_TYPE_BUDGET_REVISION_PENDING);
        final String PROJECT_TYPE_OTHER = parameterService.getParameterValue(RoutingFormDocument.class, CGConstants.PROJECT_TYPE_OTHER);
        final String PURPOSE_RESEARCH = parameterService.getParameterValue(RoutingFormDocument.class, CGConstants.PURPOSE_RESEARCH);
        final String PURPOSE_OTHER = parameterService.getParameterValue(RoutingFormDocument.class, CGConstants.PURPOSE_OTHER);
        ...
        if (routingFormDocument.getRoutingFormProjectTypes() == null || routingFormDocument.getRoutingFormProjectTypes().size() == 0) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormProjectTypes", CGKeyConstants.AUDIT_MAIN_PAGE_PROJECT_TYPE_REQUIRED, "mainpage.anchor3"));
        }
            ...
            for (RoutingFormProjectType routingFormProjectType : routingFormDocument.getRoutingFormProjectTypes()) {
                ...
                if (routingFormProjectType.getProjectTypeCode().equals(PROJECT_TYPE_NEW) && routingFormProjectType.isProjectTypeSelectedIndicator()) {
                    ...
                    if (ObjectUtils.isNull(routingFormDocument.getProjectTypeOtherDescription())) {
                        valid = false;
                        auditErrors.add(new AuditError("document.projectTypeOtherDescription", CGKeyConstants.AUDIT_MAIN_PAGE_PROJECT_TYPE_OTHER_REQUIRED, "mainpage.anchor3"));
                    }
                }
            }
            ...
            if (!SpringContext.getBean(ParameterService.class).getParameterEvaluator(RoutingFormDocument.class, CGConstants.PROJECT_TYPES, projectTypesString).evaluationSucceeds()) {
                valid = false;
                auditErrors.add(new AuditError("document.routingFormProjectTypes", CGKeyConstants.AUDIT_MAIN_PAGE_PROJECT_TYPE_INVALID, "mainpage.anchor3"));
            }
        }

        if (ObjectUtils.isNotNull(routingFormDocument.getRoutingFormPriorGrantNumber()) && projectTypeNew) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormPriorGrantNumber", CGKeyConstants.AUDIT_MAIN_PAGE_PROJECT_TYPE_NEW_AND_PRIOR_GRANT, "mainpage.anchor3"));
        }
        ...
        if (ObjectUtils.isNotNull(routingFormDocument.getGrantNumber()) && !(projectTypeTimeExtention || projectTypeBudgetRevisionActive || projectTypeBudgetRevisionPending || projectTypeOther)) {
            valid = false;
            auditErrors.add(new AuditError("document.grantNumber", CGKeyConstants.AUDIT_MAIN_PAGE_PROJECT_TYPE_SELECTION_AND_GRANT, "mainpage.anchor3"));
        }

        if (ObjectUtils.isNull(routingFormDocument.getRoutingFormPurposeCode())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormPurposeCode", CGKeyConstants.AUDIT_MAIN_PAGE_PURPOSE_REQUIRED, "mainpage.anchor3"));
        }
        ...
        if (PURPOSE_RESEARCH.equals(routingFormDocument.getRoutingFormPurposeCode()) && ObjectUtils.isNull(routingFormDocument.getResearchTypeCode())) {
            valid = false;
            auditErrors.add(new AuditError("document.researchTypeCode", CGKeyConstants.AUDIT_MAIN_PAGE_PURPOSE_RESEARCH_TYPE_REQUIRED, "mainpage.anchor3"));
        }
        ...
        if (PURPOSE_OTHER.equals(routingFormDocument.getRoutingFormPurposeCode()) && ObjectUtils.isNull(routingFormDocument.getRoutingFormOtherPurposeDescription())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormOtherPurposeDescription", CGKeyConstants.AUDIT_MAIN_PAGE_PURPOSE_OTHER_REQUIRED, "mainpage.anchor3"));
        }

        // Title, Lay Description, & Abstract
        if (ObjectUtils.isNull(routingFormDocument.getRoutingFormProjectTitle())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormProjectTitle", CGKeyConstants.AUDIT_MAIN_PAGE_TITLE_REQUIRED, "mainpage.anchor3"));
        }

        if (ObjectUtils.isNull(routingFormDocument.getRoutingFormLayDescription())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormLayDescription", CGKeyConstants.AUDIT_MAIN_PAGE_LAY_DESCRIPTION_REQUIRED, "mainpage.anchor3"));
        }

        if (ObjectUtils.isNull(routingFormDocument.getProjectAbstract())) {
            valid = false;
            auditErrors.add(new AuditError("document.projectAbstract", CGKeyConstants.AUDIT_MAIN_PAGE_ABSTRACT_REQUIRED, "mainpage.anchor3"));
        }
    ...
    private static boolean processRoutingFormMainPageAmountsDateAuditChecks(List<AuditError> auditErrors, RoutingFormDocument routingFormDocument) {
        ...
        if (ObjectUtils.isNull(routingFormBudget.getRoutingFormBudgetDirectAmount())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetDirectAmount", CGKeyConstants.AUDIT_MAIN_PAGE_DIRECT_REQUIRED, "mainpage.anchor3"));
        }
        if (ObjectUtils.isNull(routingFormBudget.getRoutingFormBudgetIndirectCostAmount())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetIndirectCostAmount", CGKeyConstants.AUDIT_MAIN_PAGE_INDIRECT_REQUIRED, "mainpage.anchor3"));
        }
        if (ObjectUtils.isNull(routingFormBudget.getRoutingFormBudgetStartDate())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetStartDate", CGKeyConstants.AUDIT_MAIN_PAGE_START_DATE_REQUIRED, "mainpage.anchor3"));
        }
        if (ObjectUtils.isNull(routingFormBudget.getRoutingFormBudgetEndDate())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetEndDate", CGKeyConstants.AUDIT_MAIN_PAGE_END_DATE_REQUIRED, "mainpage.anchor3"));
        }

        if (ObjectUtils.isNull(routingFormBudget.getRoutingFormBudgetTotalDirectAmount())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetTotalDirectAmount", CGKeyConstants.AUDIT_MAIN_PAGE_TOTAL_DIRECT_REQUIRED, "mainpage.anchor3"));
        }
        if (ObjectUtils.isNull(routingFormBudget.getRoutingFormBudgetTotalIndirectCostAmount())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetTotalIndirectCostAmount", CGKeyConstants.AUDIT_MAIN_PAGE_TOTAL_INDIRECT_REQUIRED, "mainpage.anchor3"));
        }
        if (ObjectUtils.isNull(routingFormBudget.getRoutingFormBudgetTotalStartDate())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetTotalStartDate", CGKeyConstants.AUDIT_MAIN_PAGE_TOTAL_START_DATE_REQUIRED, "mainpage.anchor3"));
        }
        if (ObjectUtils.isNull(routingFormBudget.getRoutingFormBudgetTotalEndDate())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetTotalEndDate", CGKeyConstants.AUDIT_MAIN_PAGE_TOTAL_END_DATE_REQUIRED, "mainpage.anchor3"));
        }
        ...
        if (routingFormBudget.getRoutingFormBudgetStartDate() != null && routingFormBudget.getRoutingFormBudgetEndDate() != null && routingFormBudget.getRoutingFormBudgetStartDate().compareTo(routingFormBudget.getRoutingFormBudgetEndDate()) >= 0) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetStartDate", CGKeyConstants.AUDIT_MAIN_PAGE_START_DATE_BEFORE_END_DATE, "mainpage.anchor3"));
        }
        if (routingFormBudget.getRoutingFormBudgetTotalStartDate() != null && routingFormBudget.getRoutingFormBudgetTotalEndDate() != null && routingFormBudget.getRoutingFormBudgetTotalStartDate().compareTo(routingFormBudget.getRoutingFormBudgetTotalEndDate()) >= 0) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetTotalStartDate", CGKeyConstants.AUDIT_MAIN_PAGE_TOTAL_START_DATE_BEFORE_TOTAL_END_DATE, "mainpage.anchor3"));
        }
        ...
        if (routingFormBudget.getRoutingFormBudgetDirectAmount() != null && routingFormBudget.getRoutingFormBudgetTotalDirectAmount() != null && routingFormBudget.getRoutingFormBudgetDirectAmount().isGreaterThan(routingFormBudget.getRoutingFormBudgetTotalDirectAmount())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetDirectAmount", CGKeyConstants.AUDIT_MAIN_PAGE_DIRECT_GREATER_TOTAL_DIRECT, "mainpage.anchor3"));
        }
        if (routingFormBudget.getRoutingFormBudgetIndirectCostAmount() != null && routingFormBudget.getRoutingFormBudgetTotalIndirectCostAmount() != null && routingFormBudget.getRoutingFormBudgetIndirectCostAmount().isGreaterThan(routingFormBudget.getRoutingFormBudgetTotalIndirectCostAmount())) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetIndirectCostAmount", CGKeyConstants.AUDIT_MAIN_PAGE_INDIRECT_GREATER_TOTAL_INDIRECT, "mainpage.anchor3"));
        }
        if (routingFormBudget.getRoutingFormBudgetStartDate() != null && routingFormBudget.getRoutingFormBudgetTotalStartDate() != null && routingFormBudget.getRoutingFormBudgetStartDate().compareTo(routingFormBudget.getRoutingFormBudgetTotalStartDate()) < 0) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetStartDate", CGKeyConstants.AUDIT_MAIN_PAGE_START_DATE_GREATER_TOTAL_START_DATE, "mainpage.anchor3"));
        }
        if (routingFormBudget.getRoutingFormBudgetEndDate() != null && routingFormBudget.getRoutingFormBudgetTotalEndDate() != null && routingFormBudget.getRoutingFormBudgetEndDate().compareTo(routingFormBudget.getRoutingFormBudgetTotalEndDate()) > 0) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetStartDate", CGKeyConstants.AUDIT_MAIN_PAGE_END_DATE_LESS_TOTAL_END_DATE, "mainpage.anchor3"));
        }
        ...
        if (routingFormBudget.getRoutingFormBudgetDirectAmount() != null && routingFormBudget.getRoutingFormBudgetDirectAmount().isLessThan(totalSubcontractorAmountInteger)) {
            valid = false;
            auditErrors.add(new AuditError("document.routingFormBudget", CGKeyConstants.AUDIT_MAIN_PAGE_SUBCONTRACTOR_TOTAL_GREATER_DIRECT, "mainpage.anchor3"));
        }
    ...
    private static void processRoutingFormMainPageSoftErrors(List<AuditError> auditErrors, RoutingFormDocument routingFormDocument) {
        ...
        if (routingFormBudget.getRoutingFormBudgetDirectAmount() != null && routingFormBudget.getRoutingFormBudgetIndirectCostAmount() != null && routingFormBudget.getRoutingFormBudgetDirectAmount().isLessThan(routingFormBudget.getRoutingFormBudgetIndirectCostAmount())) {
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetDirectAmount", CGKeyConstants.AUDIT_MAIN_PAGE_DIRECT_LESS_INDIRECT, "mainpage.anchor3"));
        }
        ...
        if (routingFormBudget.getRoutingFormBudgetTotalDirectAmount() != null && routingFormBudget.getRoutingFormBudgetTotalIndirectCostAmount() != null && routingFormBudget.getRoutingFormBudgetTotalDirectAmount().isLessThan(routingFormBudget.getRoutingFormBudgetTotalIndirectCostAmount())) {
            auditErrors.add(new AuditError("document.routingFormBudget.routingFormBudgetTotalDirectAmount", CGKeyConstants.AUDIT_MAIN_PAGE_TOTAL_DIRECT_LESS_TOTAL_INDIRECT, "mainpage.anchor3"));
        }
