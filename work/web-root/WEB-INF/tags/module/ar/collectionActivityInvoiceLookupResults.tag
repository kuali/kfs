...
<c:if test="${!empty resultsList}">
	...
	<%-- <input type="text" name="${Constants.MULTIPLE_VALUE_LOOKUP_PREVIOUSLY_SELECTED_OBJ_IDS_PARAM}" value="${KualiForm.compositeSelectedObjectIds}" /> --%>
	...
	<table cellpadding="0" class="datatable-100" cellspacing="1" id="row">
		...
		<c:forEach items="${resultsList}" var="row" varStatus="rowLoopStatus" begin="${KualiForm.firstRowIndex}" end="${KualiForm.lastRowIndex}">
		    ...
		    <input type="hidden" name="${hiddenFieldName}" value="onscreen" /> ${kfunc:registerEditableProperty(KualiForm, hiddenFieldName)}
				    </td>
