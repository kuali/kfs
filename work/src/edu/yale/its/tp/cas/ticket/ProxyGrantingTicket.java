...
/**
 * Represents a CAS proxy-granting ticket (PGT), used to retrieve proxy tickets (PTs). Note that we extend TicketGrantingTicket;
 * this doesn't mean we can act as a TGC: a TicketCache for TGCs will never store a PGT.
 */
public class ProxyGrantingTicket extends TicketGrantingTicket {
    ...
    /**
     * A PGT is constructed with a ServiceTicket (potentially a ProxyTicket). We store this ticket to inherit ticket expiration.
     */
    private ServiceTicket parent;

    /**
     * A PGT is also consturcted with the ID of the proxy. In CAS 2.0.1, this ID corresponds to the callback URL to which the PGT's
     * ID is sent.
     */
