...
public class SufficientFundsServiceTest extends KualiTestBase {
    ...
    private void insertPendingLedgerEntries(String accountNumber, String sfObjCd) {
        ...
        unitTestSqlDao.sqlCommand("delete from fp_doc_header_t where fdoc_nbr = '" + documentHeaderId + "'");
        unitTestSqlDao.sqlCommand("delete from fs_doc_header_t where fdoc_nbr = '" + documentHeaderId + "'");
        unitTestSqlDao.sqlCommand("insert into FP_DOC_HEADER_T (FDOC_NBR, OBJ_ID, VER_NBR, FDOC_DESC, ORG_DOC_NBR, FDOC_TMPL_NBR) values ('" + documentHeaderId + "','" + new Guid().toString() + "', 1, 'test', '', '')");
        unitTestSqlDao.sqlCommand("insert into FS_DOC_HEADER_T (FDOC_NBR, OBJ_ID, VER_NBR, FDOC_STATUS_CD, FDOC_TOTAL_AMT, FDOC_IN_ERR_NBR, TEMP_DOC_FNL_DT) values ('" + documentHeaderId + "','" + new Guid().toString() + "', 1, 'A', 0, '', " + unitTestSqlDao.getDbPlatform().getCurTimeFunction() + ")");
