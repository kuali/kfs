...
public abstract class WatchfulPasswordHandler implements PasswordHandler {
    ...
    /**
     * The number of failed login attempts to allow before locking out the source IP address. (Note that failed login attempts
     * "expire" regularly.)
     */
     ...
     * subclasses.
     */
    public synchronized boolean authenticate(javax.servlet.ServletRequest request, String netid, String password) {
    ...
    /**
     * Removes one failure record from each offender; if any offender's resulting total is zero, remove it from the list.
     */
