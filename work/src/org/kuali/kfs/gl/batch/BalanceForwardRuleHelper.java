...
public class BalanceForwardRuleHelper {
    ...
    /**
     * A container for the state of the balance forward process. The way state
     * is handled is heavily dependent upon the way in which YearEndServiceImpl.forwardBalancesForFiscalYear works.
     * 
     * @author Kuali General Ledger Team (kualigltech@oncourse.iu.edu)
     */
...
//  972  004970        MOVE 'Y' TO WS-SELECT-GENERAL-SW.
            ...
            if(ObjectHelper.isOneOf(
                    balance.getBalanceTypeCode(), new String[] {"AC", "NB"})
...
// 1012  005390            GLGLBL-FIN-BEG-BAL-LN-AMT) = ZERO
                ...
                if(balance.getAccountLineAnnualBalanceAmount()
                          .add(balance.getContractsGrantsBeginningBalanceAmount())
                          .add(balance.getBeginningBalanceLineAmount()).isZero()) {
...
// 1057  005840           MOVE 'Y' TO WS-FATAL-ERROR-FLAG
                      ...
                      throw new FatalErrorException(
                              new StringBuffer(" ERROR ACCESSING PRIOR YR ACCT TABLE FOR ")
                              .append("CHART ").append(balance.getChartOfAccountsCode())
                              .append(" AND ACCOUNT ").append(balance.getAccountNumber()).toString());
...
// 1076  006030                OR 'PFCMR ')
                      ...
                      if ("CG".equals(subFundGroup.getFundGroupCode())
                              || ObjectHelper.isOneOf(
                                      subFundGroup.getSubFundGroupCode().trim(), 
                                      new String[] {"SDCI", "PFCMR"})) {
...
// 1082  006090           MOVE 'Y' TO WS-FATAL-ERROR-FLAG
                      ...
                      throw new FatalErrorException(
                              new StringBuffer(" ERROR ACCESSING SUB FUND GROUP TABLE FOR SUB FUND GROUP ")
                              .append(priorYearAccount.getSubFundGroupCode()).toString());
...
// 1125  006520            GLGLBL-CONTR-GR-BB-AC-AMT) = ZERO
                  ...
                  if(balance.getAccountLineAnnualBalanceAmount().add(
                          balance.getContractsGrantsBeginningBalanceAmount()).isZero()) {
...
//      953  004760     IF GLGLBL-ACCOUNT-NBR = WS-ACCOUNT-NBR-HOLD
              ...
              if((null == balance.getAccountNumber() && null == state.getAccountNumberHold())
                      || (null != balance.getAccountNumber() 
                              && balance.getAccountNumber().equals(state.getAccountNumberHold()))) {
...
//          1165  006920           WRITE PRINT-DATA
                      ...
                      LOG.info(
                              ...
                              .append(balance.getBalanceTypeCode())
                              .append(" NOT ON TABLE "));
...
//          1181  007080     END-EXEC.
                  ...
                  String balanceObjectTypeDebitCreditCode = 
                      null != balance.getObjectType() 
...
//          1187  007140        IF CAOTYP-FIN-OBJTYP-DBCR-CD = 'C' OR 'D'
                      ...
                      if(ObjectHelper.isOneOf(balanceObjectTypeDebitCreditCode,
                              new String[] {
...
//          1189  007160             TO WS-FIN-OBJTYP-DBCR-CD
                          ...
                          wsFinancialObjectTypeDebitCreditCode = 
                              balanceObjectTypeDebitCreditCode;
...
//          1205  007320           WRITE PRINT-DATA
                      ...
                      LOG.info(new StringBuffer("FIN OBJ TYP CODE ")
...
//              1228  007550       TO UNIV-FISCAL-YR OF GLEN-RECORD.
                      ...
                      entry.setUniversityFiscalYear(
                              new Integer(closingFiscalYear.intValue() + 1));
...
//              1246  007730          TO FIN-OBJ-TYP-CD.
                          ...
                          entry.setFinancialObjectTypeCode(
                                  balance.getObjectTypeCode());
...
//              1257  007840       INTO FDOC-NBR.
                      ...
                      entry.setFinancialDocumentNumber(
                              new StringBuffer("AC").append(
                                      balance.getAccountNumber())/*.append(balance.getChartOfAccountsCode())*/.toString());
                      
//              1258  007850     MOVE WS-SEQ-NBR
//              1259  007860       TO TRN-ENTR-SEQ-NBR.
                      
                      entry.setTransactionLedgerEntrySequenceNumber(
                              new Integer(state.getSequenceNumber()));
...
//              1264  007910            TRN-LDGR-ENTR-DESC.
                      ...
                      entry.setTransactionLedgerEntryDescription(
                              new StringBuffer("BEG BAL BROUGHT FORWARD FROM ")
                              .append(closingFiscalYear).toString());
...
//              1268  007950     GIVING TRN-LDGR-ENTR-AMT.
                      ...
                      KualiDecimal transactionLedgerEntryAmount = 
                          new KualiDecimal(0);
                      transactionLedgerEntryAmount = transactionLedgerEntryAmount
...
//              1300  008270          = TRN-LDGR-ENTR-AMT * -1.
                              ...
                              transactionLedgerEntryAmount = 
                                  transactionLedgerEntryAmount.negated();
...
//              1301  008280     IF CAPYACTT-ACCT-CLOSED-IND NOT = 'Y'
                      ...
                      if(null != priorYearAccount 
                              && !priorYearAccount.isAccountClosedIndicator()) {
...
//              1305  008320       WRITE GLE-DATA FROM GLEN-RECORD
                          ...
                          originEntryService.createEntry(
                                  entry, unclosedPriorYearAccountGroup);
...
//              1316                  DISPLAY '  SEQUENTIAL RECORDS WRITTEN = ' SEQ-CHECK-CNT
                              ...
                              LOG.info("  SEQUENTIAL RECORDS WRITTEN = " 
                                      + state.getSequenceWriteCount());
...
//              1360  008820       INTO FDOC-NBR                     OF GLEN-RECORD.
                      ...
                      activeEntry.setFinancialDocumentNumber(
                              new StringBuffer("AC")
                              .append(balance.getAccountNumber()).toString());
                      
//              1361  008830     MOVE WS-SEQ-NBR
//              1362  008840       TO TRN-ENTR-SEQ-NBR               OF GLEN-RECORD.
              
                      activeEntry.setTransactionLedgerEntrySequenceNumber(
                              new Integer(state.getSequenceNumber()));
...
//              1367  008890            TRN-LDGR-ENTR-DESC OF GLEN-RECORD.
                      ...
                      activeEntry.setTransactionLedgerEntryDescription(
                              new StringBuffer("BEG C & G BAL BROUGHT FORWARD FROM ")
                              .append(closingFiscalYear).toString());
...
//              1370  008920     GIVING TRN-LDGR-ENTR-AMT              OF GLEN-RECORD.
                      ...
                      activeEntry.setTransactionLedgerEntryAmount(
                              balance.getAccountLineAnnualBalanceAmount().add(
...
//              1376  008980           IF WS-FIN-OBJTYP-DBCR-CD = 'C'
                              ...
                              if(Constants.GL_CREDIT_CODE.equals(
                                      wsFinancialObjectTypeDebitCreditCode)) {
                                  
//              1377  008990              MOVE 'D'
//              1378  009000                TO TRN-DEBIT-CRDT-CD  OF GLEN-RECORD
                                  
                                  activeEntry.setTransactionDebitCreditCode(
                                          Constants.GL_DEBIT_CODE);
...
//              1402  009240       TO TRN-ENCUM-UPDT-CD              OF GLEN-RECORD.
                      ...
                      activeEntry.setTransactionEncumbranceUpdateCode(
                              entry.getTransactionEncumbranceUpdateCode());
...
//              1408  009300          = TRN-LDGR-ENTR-AMT OF GLEN-RECORD * -1.
                              ...
                              activeEntry.setTransactionLedgerEntryAmount(
                                      activeEntry.getTransactionLedgerEntryAmount().negated());
