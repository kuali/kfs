    ...
    public Collection performLookup(LookupForm lookupForm, Collection resultTable, boolean bounded) {
        ...
        for (CashControlDocument cashControlDoc : cgCashControlDocs) {
            ...
            for (CashControlDetail cashControlDetail : cashControlDetailList) {
                ...
                try {
                    isFinal = paymentApplicationDoc.getDocumentHeader().getWorkflowDocument().stateIsFinal();
                }
                catch (Exception e) {
                ...
                if (isFinal) {
                    ...
                    for (InvoicePaidApplied appliedPayment : appliedPayments) {
                        ...
                        cgPaymentHistoryReport.setPaymentDate(DateUtils.convertToSqlDate(paymentApplicationDoc.getDocumentHeader().getWorkflowDocument().getRouteHeader().getDateFinalized().getTime()));
                        ...
                        String reversedInd = appliedPayment.getCustomerInvoiceDocument().isInvoiceReversal() ? "Yes" : "No";
                        cgPaymentHistoryReport.setReversedIndicator(reversedInd);

                        cgPaymentHistoryReport.setAppliedIndicator("Yes");
