			...
			<tr>
				<td>
					<div class="floaters">
						...
						<p align="center">
							<c:choose>
								<c:when test="${KualiForm.auditActivated}"><html:image title="deactivate" alt="deactivate" property="methodToCall.deactivate" src="${ConfigProperties.externalizable.images.url}tinybutton-deacaudit.gif" styleClass="tinybutton" /></c:when>
								<c:otherwise><html:image title="activate" alt="activate" property="methodToCall.activate" src="${ConfigProperties.externalizable.images.url}tinybutton-activaudt.gif" styleClass="tinybutton" /></c:otherwise>
							</c:choose>
						</p>
					</div>
				</td>
