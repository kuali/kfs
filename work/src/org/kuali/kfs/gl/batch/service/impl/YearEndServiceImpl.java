...
//        683  003700      ACCEPT VAR-NET-EXP-OBJECT-CD FROM ENVIRONMENT-VALUE.
        ...
        try {
            ...
            varNetExpenseObjectCode = 
                kualiConfigurationService.getApplicationParameterValue(
                        "fis_gl_year_end.sh", "NET_EXP_OBJECT_CD");
            
        } catch (ApplicationParameterException e) {
            
            LOG.error(
                    "Unable to get NET_EXP_OBJECT_CD from kualiConfigurationService");
            throw new RuntimeException(
                    "Unable to get net expense object code from kualiConfigurationService", e);
...
//        685  003720      ACCEPT VAR-NET-REV-OBJECT-CD FROM ENVIRONMENT-VALUE.
        ...
        try {
            ...
            varNetRevenueObjectCode = 
                kualiConfigurationService.getApplicationParameterValue(
                        "fis_gl_year_end.sh", "NET_REV_OBJECT_CD");
            
        } catch (ApplicationParameterException e) {
            
            LOG.error(
                    "Unable to get NET_REV_OBJECT_CD from kualiConfigurationService");
            throw new RuntimeException(
                    "Unable to get net revenue object code from kualiConfigurationService", e);
...
//        687  003740      ACCEPT VAR-FUND-BAL-OBJECT-CD FROM ENVIRONMENT-VALUE.
        ...
        try {
            ...
            varFundBalanceObjectCode =
                kualiConfigurationService.getApplicationParameterValue(
                        "fis_gl_year_end.sh", "FUND_BAL_OBJECT_CD");
            
        } catch(ApplicationParameterException e) {
            
            LOG.error(
                    "Unable to get FUND_BAL_OBJECT_CD from kualiConfigurationService");
            throw new RuntimeException(
                    "Unable to get fund balance object code from kualiConfigurationService", e);
...
//        689  003760      ACCEPT VAR-FUND-BAL-OBJ-TYP-CD FROM ENVIRONMENT-VALUE.
        ...
        try {
            ...
            varFundBalanceObjectTypeCode =
                kualiConfigurationService.getApplicationParameterValue(
                        "fis_gl_year_end.sh", "FUND_BAL_OBJ_TYP_CD");
            
        } catch(ApplicationParameterException e) {

            LOG.error(
                    "Unable to get FUND_BAL_OBJECT_TYP_CD from kualiConfigurationService");
            throw new RuntimeException(
...
//                952  006330       TO TRN-LDGR-ENTR-AMT                 OF GLEN-RECORD.
                    ...
                    activityEntry.setTransactionLedgerEntryAmount(
...
//                975  006530         IF TRN-LDGR-ENTR-AMT OF GLEN-RECORD < ZERO
                        ...
                        // NOTE (laran) I'm using the balance value here because to me it was easier than remembering the indirection.
...
//                976  006540            IF CAOTYP-FIN-OBJTYP-DBCR-CD = 'C' OR 'D'
                            ...
                            if(ObjectHelper.isOneOf(
                                    balance.getObjectType().getFinObjectTypeDebitcreditCd(), 
                                    new String[]{Constants.GL_CREDIT_CODE, Constants.GL_DEBIT_CODE})) {
...
//               1021  006990           WRITE PRINT-DATA
                        ...
                        LOG.info(
                                new StringBuffer("FIN OBJ TYP CODE ")
                                .append(balance.getObjectTypeCode())
                                .append(" NOT IN TABLE").toString());
...
//               1065                DISPLAY '  SEQUENTIAL RECORDS WRITTEN = ' SEQ-CHECK-CNT.
                        ...
                        LOG.info(
                                new StringBuffer("  SEQUENTIAL RECORDS WRITTEN = " )
...
//                1096  007710       INTO FDOC-NBR                     OF GLEN-RECORD.
                    ...
                    offsetEntry.setFinancialDocumentNumber(
...
//                1098  007730       TO TRN-ENTR-SEQ-NBR               OF GLEN-RECORD.
                    ...
                    offsetEntry.setTransactionLedgerEntrySequenceNumber(
                            new Integer(sequenceNumber));
...
//        812  004950                           OR 'EE' OR 'CH' OR 'IC' OR 'IN')
        ...
        if("AC".equals(balance.getBalanceTypeCode())
                && ObjectHelper.isOneOf(
                        ...
                        new String[] {"ES", "EX", "TE", "TI", "EE", "CH", "IC", "IN"})) {
...
//        837  005200        GO TO 3000-SELECT-CRITERIA-EXIT.
    ...
    /**
     * This method handles executing the loop over all balances, and generating
     * reports on the balance forwarding process as a whole. This method delegates
     * all of the specific logic in terms of what balances to forward, according
     * to what criteria, how origin entries are generated, etc. This relationship
     * makes YearEndServiceImpl and BalanceForwardRuleHelper heavily dependent upon
     * one another in terms of expected behavior.
     */
    public void forwardBalances() {
        ...
        OriginEntryGroup unclosedPriorYearAccountGroup = 
            originEntryGroupService.createGroup(
                    varTransactionDate, "YEBB", false, false, false);
        OriginEntryGroup closedPriorYearAccountGroup = 
            originEntryGroupService.createGroup(
                    varTransactionDate, "YEBC", false, false, false);;
        
        BalanceForwardRuleHelper balanceForwardRuleHelper = 
            new BalanceForwardRuleHelper(
