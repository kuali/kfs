...
<c:if test="${!KualiForm.canUnmask && !empty attributeEntry.attributeSecurityMask && attributeEntry.attributeSecurityMask == true  }">
...
<c:if test="${!KualiForm.canUnmask && !displayMask && !empty attributeEntry.attributeSecurityPartialMask && attributeEntry.attributeSecurityPartialMask == true  }">
	<c:set var="className" value ="${attributeEntry.fullClassName}" />
	<c:set var="fieldName" value ="${attributeEntry.name}" />
...
<%-- Allows the masking to be hijacked by special circumstances--%>
