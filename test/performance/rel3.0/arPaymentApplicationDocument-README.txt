...
- the invoice due date must not be before or equal to the billing date.  In the business rules class, the billing date is defined as the current date, so when the jMeter test is run, the parameter "invoiceDueDate" must be defined as today's date plus one.  (ie. today is 6/10/09, then the parameter must be set to 6/11/09 or later).  
