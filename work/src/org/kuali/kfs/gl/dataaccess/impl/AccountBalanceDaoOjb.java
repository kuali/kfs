...
public class AccountBalanceDaoOjb extends PersistenceBrokerDaoSupport implements AccountBalanceDao {
    ...
    /**
     * This is based on the code in the FIS. Unfortunately, there is business logic in here. This should be cleaned up at some
     * point.
     * 
    ...
    public List findAccountBalanceByConsolidation(Integer universityFiscalYear, String chartOfAccountsCode, String accountNumber, boolean isExcludeCostShare, boolean isConsolidated) {
        ...
        sqlCommand("INSERT INTO fp_interim1_cons_mt (UNIV_FISCAL_YR, FIN_COA_CD, ACCOUNT_NBR, SUB_ACCT_NBR, FIN_OBJECT_CD, FIN_SUB_OBJ_CD, CURR_BDLN_BAL_AMT, ACLN_ACTLS_BAL_AMT, " + "ACLN_ENCUM_BAL_AMT, TIMESTAMP, FIN_REPORT_SORT_IND, FIN_OBJ_TYP_CD, SESID, OBJ_ID, VER_NBR) SELECT A.UNIV_FISCAL_YR, A.FIN_COA_CD, A.ACCOUNT_NBR, A.SUB_ACCT_NBR, " + "A.FIN_OBJECT_CD, A.FIN_SUB_OBJ_CD, A.CURR_BDLN_BAL_AMT, A.ACLN_ACTLS_BAL_AMT, A.ACLN_ENCUM_BAL_AMT, A.TIMESTAMP, SUBSTR(fin_report_sort_cd, 1, 1), " + "t.fin_obj_typ_cd, USERENV('SESSIONID'), sys_guid(), 1 FROM gl_acct_balances_t a, ca_object_code_t o, ca_obj_type_t t WHERE a.univ_fiscal_yr = " + universityFiscalYear + " AND a.fin_coa_cd = '" + chartOfAccountsCode + "' AND a.account_nbr = '" + accountNumber + "' AND a.univ_fiscal_yr = o.univ_fiscal_yr AND a.fin_coa_cd = o.fin_coa_cd " + " AND a.fin_object_cd = o.fin_object_cd AND o.fin_obj_typ_cd = t.fin_obj_typ_cd AND o.univ_fiscal_yr = " + universityFiscalYear + " AND o.fin_coa_cd = '"
                + chartOfAccountsCode + "' " + " AND o.fin_obj_typ_cd IN ('EE', 'EX', 'ES', 'IN', 'IC', 'CH')");

        sqlCommand("INSERT INTO fp_interim2_cons_mt (UNIV_FISCAL_YR, FIN_COA_CD, ACCOUNT_NBR, SUB_ACCT_NBR, FIN_OBJECT_CD, FIN_SUB_OBJ_CD, CURR_BDLN_BAL_AMT, ACLN_ACTLS_BAL_AMT, " + "ACLN_ENCUM_BAL_AMT, TIMESTAMP, FIN_REPORT_SORT_IND,FIN_OBJ_TYP_CD, SESID, CONS_FIN_REPORT_SORT_CD, FIN_CONS_OBJ_CD, OBJ_ID, VER_NBR) " + "SELECT A.UNIV_FISCAL_YR, A.FIN_COA_CD, A.ACCOUNT_NBR, A.SUB_ACCT_NBR,A.FIN_OBJECT_CD, A.FIN_SUB_OBJ_CD, A.CURR_BDLN_BAL_AMT, A.ACLN_ACTLS_BAL_AMT, " + "A.ACLN_ENCUM_BAL_AMT, A.TIMESTAMP, A.FIN_REPORT_SORT_IND, A.FIN_OBJ_TYP_CD, A.SESID,c.fin_report_sort_cd,c.fin_cons_obj_cd,sys_guid(), 1 " + "FROM fp_interim1_cons_mt a,ca_object_code_t o,ca_obj_level_t l,ca_obj_consoldtn_t c WHERE a.univ_fiscal_yr = o.univ_fiscal_yr " + "AND a.fin_coa_cd = o.fin_coa_cd AND a.fin_object_cd = o.fin_object_cd AND o.fin_coa_cd = l.fin_coa_cd AND o.fin_obj_level_cd = l.fin_obj_level_cd " + "AND c.fin_coa_cd = l.fin_coa_cd AND c.fin_cons_obj_cd = l.fin_cons_obj_cd AND o.univ_fiscal_yr = "
        ...
        if (isExcludeCostShare) {
            sqlCommand("DELETE fp_interim2_cons_mt WHERE ROWID IN (SELECT i.ROWID FROM fp_interim2_cons_mt i,ca_a21_sub_acct_t a WHERE (a.fin_coa_cd = i.fin_coa_cd " + "AND a.account_nbr = i.account_nbr AND a.sub_acct_nbr = i.sub_acct_nbr AND a.sub_acct_typ_cd = 'CS') AND fp_interim2_cons_mt.SESID = USERENV('SESSIONID'))");
        }

        // Summarize
        if (isConsolidated) {
            sqlCommand("INSERT INTO fp_bal_by_cons_t (SUB_ACCT_NBR, FIN_REPORT_SORT_CD, CONS_FIN_REPORT_SORT_CD, FIN_CONS_OBJ_CD, CURR_BDLN_BAL_AMT, ACLN_ACTLS_BAL_AMT, " + "ACLN_ENCUM_BAL_AMT, OBJ_ID, VER_NBR, PERSON_SYS_ID) SELECT '*ALL*',fin_report_sort_ind,cons_fin_report_sort_cd,fin_cons_obj_cd,SUM(curr_bdln_bal_amt), " + "SUM(acln_actls_bal_amt), SUM(acln_encum_bal_amt), sys_guid(), 1, USERENV('SESSIONID') FROM fp_interim2_cons_mt WHERE fp_interim2_cons_mt.SESID = USERENV('SESSIONID') " + "GROUP BY cons_fin_report_sort_cd, fin_report_sort_ind, fin_cons_obj_cd");
        }
        else {
            sqlCommand("INSERT INTO fp_bal_by_cons_t (SUB_ACCT_NBR, FIN_REPORT_SORT_CD, CONS_FIN_REPORT_SORT_CD, FIN_CONS_OBJ_CD, CURR_BDLN_BAL_AMT, ACLN_ACTLS_BAL_AMT, " + "ACLN_ENCUM_BAL_AMT, OBJ_ID, VER_NBR, PERSON_SYS_ID) SELECT sub_acct_nbr, fin_report_sort_ind, cons_fin_report_sort_cd, fin_cons_obj_cd, SUM(curr_bdln_bal_amt), " + "SUM(acln_actls_bal_amt), SUM(acln_encum_bal_amt), sys_guid(), 1, USERENV('SESSIONID') FROM fp_interim2_cons_mt WHERE fp_interim2_cons_mt.SESID = USERENV('SESSIONID') " + "GROUP BY sub_acct_nbr, cons_fin_report_sort_cd, fin_report_sort_ind, fin_cons_obj_cd");
        }
    ...
    public List findAccountBalanceByLevel(Integer universityFiscalYear, String chartOfAccountsCode, String accountNumber, String financialConsolidationObjectCode, boolean isCostShareExcluded, boolean isConsolidated) {
        ...
        sqlCommand("INSERT INTO fp_interim1_level_mt (UNIV_FISCAL_YR, FIN_COA_CD, ACCOUNT_NBR, SUB_ACCT_NBR, FIN_OBJECT_CD, FIN_SUB_OBJ_CD, CURR_BDLN_BAL_AMT, " + "ACLN_ACTLS_BAL_AMT, ACLN_ENCUM_BAL_AMT, TIMESTAMP, FIN_REPORT_SORT_CD, FIN_OBJ_LEVEL_CD, SESID, OBJ_ID, VER_NBR) SELECT A.UNIV_FISCAL_YR, A.FIN_COA_CD, " + "A.ACCOUNT_NBR, A.SUB_ACCT_NBR,A.FIN_OBJECT_CD, A.FIN_SUB_OBJ_CD,A.CURR_BDLN_BAL_AMT, A.ACLN_ACTLS_BAL_AMT, A.ACLN_ENCUM_BAL_AMT, A.TIMESTAMP, " + "fin_report_sort_cd, l.fin_obj_level_cd, USERENV('SESSIONID'), sys_guid(), 1 FROM gl_acct_balances_t a, ca_object_code_t o, ca_obj_level_t l " + "WHERE a.univ_fiscal_yr = " + universityFiscalYear + " AND a.fin_coa_cd = '" + chartOfAccountsCode + "' AND a.account_nbr = '" + accountNumber + "' " + "AND a.univ_fiscal_yr = o.univ_fiscal_yr AND a.fin_coa_cd = o.fin_coa_cd AND a.fin_object_cd = o.fin_object_cd AND o.fin_coa_cd = l.fin_coa_cd " + "AND o.fin_obj_level_cd = l.fin_obj_level_cd AND l.fin_cons_obj_cd = '"
                + financialConsolidationObjectCode + "' AND o.univ_fiscal_yr = " + universityFiscalYear + " " + "AND o.fin_coa_cd = '" + chartOfAccountsCode + "'");
