...
public class ShippingHeaderHistory extends ShippingHeader {
    ...
    public List<ShippingInvoice> getInvoices() {
        if ( !invoicesLoaded ) {
            ...
            criteria.put("loadDate", loadDate);
            invoices = new ArrayList<ShippingInvoice>( SpringContext.getBean(BusinessObjectService.class).findMatching(ShippingInvoiceHistory.class,criteria) );
            invoicesLoaded = true;
        }        
        return invoices;
    }
