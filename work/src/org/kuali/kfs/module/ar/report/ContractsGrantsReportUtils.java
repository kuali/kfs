...
public class ContractsGrantsReportUtils {
    ...
    public static boolean doesMatchLookupFields(Map fieldsForLookup, BusinessObject bo, String className) {
        BusinessObjectEntry boe = KNSServiceLocator.getDataDictionaryService().getDataDictionary().getBusinessObjectEntry(className);
        ...
        for (Object propertyName : fieldsForLookup.keySet()) {
            ...
            if (lookupResultFields.contains(propertyName.toString())) {
                ...
                if (!(propertyValueObject instanceof Date)) {
                    ...
                    if (isDateFieldInRange(fieldsForLookup, (Date) propertyValueObject, propertyName.toString())) {
                        returnBoolean &= true;
                    }
                    else {
                        return false;
    ...
    private static boolean isDateFieldInRange(Map fieldsForLookup, Date dateData, String fieldName) {
        ...
        String dateFromFieldValues = ObjectUtils.isNull(fieldsForLookup.get(KNSConstants.LOOKUP_RANGE_LOWER_BOUND_PROPERTY_PREFIX + fieldName)) ? "" : fieldsForLookup.get(KNSConstants.LOOKUP_RANGE_LOWER_BOUND_PROPERTY_PREFIX + fieldName).toString();
