...
public class BalanceServiceImpl implements BalanceService {
     ...
     * 
     * 
     * Here is an excerpt from the original Oracle trigger:
     SELECT  fin_object_cd
     FROM gl_balance_t 
     WHERE univ_fiscal_yr = p_univ_fiscal_yr 
     AND fin_coa_cd = p_fin_coa_cd 
     AND account_nbr = p_account_nbr 
     AND fin_object_cd != '9899' 
     AND fin_obj_typ_cd IN ('AS', 'LI', 'FB') 
     ...
     to be closed when it shouldn't be.
     ...
     * 
        ...
        AND 
        (fin_object_cd = '9899' 
        OR fin_obj_typ_cd IN ('CH', 'IC', 'IN', 'TI')) 
        AND fin_balance_typ_cd = 'AC';
     * @return
     */
    ...
    * Here is an excerpt from the original Oracle Trigger:   
        ...
        AND fin_coa_cd = p_fin_coa_cd 
        AND account_nbr = p_account_nbr 
        AND fin_obj_typ_cd IN ('EE', 'ES', 'EX', 'TE') 
        AND fin_balance_typ_cd = 'AC';
     ...
     * Here is an excerpt from the original Oracle Trigger:         
        ...
        AND fin_coa_cd = p_fin_coa_cd 
        AND account_nbr = p_account_nbr 
        AND fin_balance_typ_cd IN ('EX', 'IE', 'PE', 'BB');
