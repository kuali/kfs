...
<c:if test="${!empty KualiForm.document.currentTransaction.openItemsInProcess}">
  <table border="0" cellspacing="0" cellpadding="0" class="datatable">
    ...
    <cm:miscAdvanceHeader itemInProcessProperty="document.currentTransaction.openItemInProcess[${loopStatus.index}]" creatingItemInProcess="false" />
    <c:forEach var="itemInProcess" items="${KualiForm.document.currentTransaction.openItemsInProcess}" varStatus="loopStatus">
      <cm:miscAdvanceLine itemInProcessProperty="document.currentTransaction.openItemInProcess[${loopStatus.index}]" creatingItemInProcess="false" />
    </c:forEach>
  </table>
</c:if>
