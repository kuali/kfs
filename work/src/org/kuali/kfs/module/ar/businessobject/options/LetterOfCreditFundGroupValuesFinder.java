...
public class LetterOfCreditFundGroupValuesFinder extends KeyValuesBase {
    ...
    public List getKeyValues() {
        ...
        Collection<ContractsAndGrantsLetterOfCreditFundGroup> codes = SpringContext.getBean(KeyValuesService.class).findAll(ContractsAndGrantsLetterOfCreditFundGroup.class);

        List<KeyValue> labels = new ArrayList<KeyValue>();
        labels.add(new ConcreteKeyValue("", ""));

        for (ContractsAndGrantsLetterOfCreditFundGroup code : codes) {
            if (code.isActive()) {
