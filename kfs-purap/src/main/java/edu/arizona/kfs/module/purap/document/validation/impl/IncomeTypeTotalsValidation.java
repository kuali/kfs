 ...
 */
public class IncomeTypeTotalsValidation extends GenericValidation {
    private static org.apache.log4j.Logger LOG = org.apache.log4j.Logger.getLogger(IncomeTypeTotalsValidation.class);
    ...
    public boolean validate(AttributedDocumentEvent event) {
        ...
        if (!isValid && GlobalVariables.getUserSession().getPrincipalName().equals(KFSConstants.SYSTEM_USER)) {
            LOG.info("validateIncomeTypeTotals() Document " + purchasingDocument.getPurapDocumentIdentifier() + ", " + ERROR_INVALID_INCOME_TYPES_TOTAL_AMOUNT);
            isValid = true;
        }
        return isValid;
    }
    
    public boolean validatePaymentRequest(PaymentRequestDocument preqDocument) {
        boolean isValid = true;
        ...
        if (hasIncomeTypes) {
            ...
            if (preqDocument.isUseTaxIndicator()) {
                if (preqDocument.getTotalPreTaxDollarAmountAllItems(excludeArray).compareTo(incomeTypesTotal) != 0) {
                    GlobalVariables.getMessageMap().putError(KFSPropertyConstants.DOCUMENT + "incomeTypes", ERROR_INVALID_INCOME_TYPES_TOTAL_AMOUNT);
                    isValid = false;
                }
            }
