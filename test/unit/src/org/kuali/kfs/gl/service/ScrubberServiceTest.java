...
public class ScrubberServiceTest extends KualiTestBaseWithSpring {
	...
	*/
	...
	public void testClosedFiscalYear() throws Exception {
		...
		List groups = unitTestSqlDao
				.sqlSelect("select * from gl_origin_entry_grp_t order by origin_entry_grp_src_cd");
		...
		assertEquals("Transaction " + entry.getEntryId() + " wrong",
				outputTransactions[0].trim(), entry.getLine().trim());

		assertTrue("Not enough transactions 4", iter.hasNext());
		entry = (OriginEntry) iter.next();
		assertEquals("Transaction " + entry.getEntryId() + " wrong",
				outputTransactions[1].trim(), entry.getLine().trim());
