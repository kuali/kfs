...
public class ScrubberProcess {
    ...
    private void performDemerger(OriginEntryGroup errorGroup, OriginEntryGroup validGroup) {
        ...
        // TODO Write this
        // originEntryService.removeScrubberDocumentEntries(validGroup, errorGroup, expiredGroup, documentNumber, documentTypeCode,
        // originCode);
//
    ...
    private void processGroup(OriginEntryGroup originEntryGroup) {
        ...
        while ( entries.hasNext() ) {
            ...
            if ( ! isFatal(errors) ) {
                ...
                if ( errors.size() > 0 ) {
                    scrubberReportErrors.put(scrubbedEntry,errors);
                }
...
//              3350  022760         AND WS-FUND-GRP-CD = 'CG'
        ...
        if (ObjectHelper.isOneOf(workingEntry.getFinancialObjectTypeCode(), validObjectTypeCodesForCostSharing) && ObjectHelper.isOneOf(workingEntry.getFinancialBalanceTypeCode(), validBalanceTypeCodesForCostSharing) && workingEntry.getAccount().isInCg()) {
...
//        4233  032610              MOVE 'NO OBJECT FOR OBJECT ON OFSD' TO RP-MSG-ERROR
            ...
            addTransactionError(kualiConfigurationService.getPropertyString(KeyConstants.ERROR_OFFSET_DEFINITION_OBJECT_CODE_NOT_FOUND), objectCodeKey.toString(), errors);
...
//        4420  034570     MOVE +0           TO SCRB-COST-SHARE-AMOUNT.
    ...
    private String offsetString() {
        ...
        return "***" + nf.format(runCal.get(Calendar.MONTH) + 1) + nf.format(runCal.get(Calendar.DAY_OF_MONTH));
    }
...
//        5202  041830      END-EVALUATE        
    ...
    private List generateCostShareEncumbranceEntries(OriginEntry scrubbedEntry) {
        ...
        if (ifNullAddTransactionErrorAndReturnFalse(objectCode, errors, kualiConfigurationService.getPropertyString(KeyConstants.ERROR_NO_OBJECT_FOR_OBJECT_ON_OFSD), costShareEncumbranceOffsetEntry.getFinancialObjectCode())) {
...
//        4504                  PERFORM WRITE-ERROR-LINE THRU WRITE-ERROR-LINE-EXIT
            ...
            addTransactionError(kualiConfigurationService.getPropertyString(KeyConstants.ERROR_OBJECT_CODE_NOT_FOUND), originEntry.getFinancialObjectCode(), errorList);
...
//      4538                    GO TO SET-OBJECT-VERIFY.
        ...
        else if ("FINA".equals(originEntryObjectLevelCode) && ("9954".equals(originEntryObjectCode) || "5400".equals(originEntryObjectCode))) { // STUDENT_FINANCIAL_AID
                                                                                                                                                // -
                                                                                                                                                // TRSFRS_OF_FUNDS_FEE_REM
                                                                                                                                                // -
                                                                                                                                                // GRADUATE_FEE_REMISSIONS
...
//        4021  030300              GO TO 2000-ENTRY-EXIT
            ...
            addTransactionError(kualiConfigurationService.getPropertyString(KeyConstants.ERROR_OFFSET_DEFINITION_NOT_FOUND), sb.toString(), errors);
...
//        4072  030730        END-EVALUATE
            ...
            addTransactionError(kualiConfigurationService.getPropertyString(KeyConstants.ERROR_OFFSET_DEFINITION_OBJECT_CODE_NOT_FOUND), scrubbedEntry.getFinancialObjectCode(), errors);
        }
