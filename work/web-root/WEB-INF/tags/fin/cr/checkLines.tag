    ...
    <kul:tab tabTitle="Check Detail" defaultOpen="true" tabErrorKey="${KFSConstants.EDIT_CASH_RECEIPT_CHECK_DETAIL_ERRORS}" >
        <div class="tab-container" align=center>
            ...
            <table cellpadding=0 class="datatable" summary="check detail information">
    			...
    			<c:if test="${!readOnly}">
                <fin:checkLine readOnly="${readOnly}" rowHeading="add" propertyName="newCheck" actionImage="${ConfigProperties.kr.externalizable.images.url}tinybutton-add1.gif" actionAlt="add" actionMethod="addCheck" cssClass="infoline" displayHidden="${displayHidden}" />
                </c:if>            
                <logic:iterate id="check" name="KualiForm" property="document.checks" indexId="ctr">
                    <fin:checkLine readOnly="${readOnly}" rowHeading="${ctr + 1}" propertyName="document.check[${ctr}]" baselinePropertyName="baselineCheck[${ctr}]" actionImage="${ConfigProperties.kr.externalizable.images.url}tinybutton-delete1.gif" actionAlt="delete" actionMethod="deleteCheck.line${ctr}" cssClass="datacell" displayHidden="${displayHidden}" />
                </logic:iterate>
...
<c:if test="${!checkDetailMode}">
    <kul:hiddenTab forceOpen="true">
        ...
        <logic:iterate id="check" name="KualiForm" property="document.checks" indexId="ctr">
            <fin:hiddenCheckLine propertyName="document.check[${ctr}]" baselinePropertyName="baselineCheck[${ctr}]"  displayHidden="${displayHidden}" />
        </logic:iterate>
    </kul:hiddenTab>
</c:if>
