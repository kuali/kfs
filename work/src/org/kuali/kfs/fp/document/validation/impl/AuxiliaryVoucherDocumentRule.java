...
public class AuxiliaryVoucherDocumentRule extends TransactionalDocumentRuleBase {
     ...
     * of the accounting lines and also those checks that would normally be done on a save, automatically for us.<br/>
     * <br/>
     * <code>{@link AuxiliaryVoucherDocument}</code> is different from other <code>{@link Document}</code> instances because it 
     * requires all <code>{@link AccountingLine}</code> instances belong to the same Fund Group. That's done here by iterating through 
     * the <code>{@link AccountingLine}</code> instances.
     * 
    ...
    /**
     * Iterates <code>{@link AccountingLine}</code> instances in a given <code>{@link TransactionalDocument}</code> 
     * instance and compares them to see if they are all in the same Fund Group.
     * 
