 ...
 * Voucher Document instances. This class is unique in that it leverages a helper data structure called the 
 * <code>{@link VoucherAccountingLineHelper}</code> because Voucher documents, under some/none conditions, presents the 
 * user with a debit and credit column for amount entry. New accounting lines use specific credit and debit amount fields b/c
...
public class VoucherForm extends KualiTransactionalDocumentFormBase {
	...
	/**
	 * Accessor to the list of <code>{@link VoucherLineHelper}</code> instances. This method retrieves the list of helper line objects for the form.
	 *
	 * @return List
	 */
     ...
     * a basic AccountingPeriod object so that the voucher is properly persisted 
    ...
    /**
     * This method uses the newly entered debit and credit amounts to populate the new source line that is to be added to the voucher
     * document.
     * 
    ...
    /**
     * This method iterates through all of the source accounting lines associated with the voucher doc and accounts for any changes to
    ...
    protected boolean processDebitAndCreditForAllSourceLines() {
        ...
        for (int i = 0; i < vDoc.getSourceAccountingLines().size(); i++) {
            ...
            validProcessing &= processDebitAndCreditForSourceLine(sourceLine, 
																  voucherLineHelper.getDebit(), 
																  voucherLineHelper.getCredit(), i);
        }
     ...
     * @return boolean True if the processing was successful, false otherwise.
     */
    protected boolean processDebitAndCreditForSourceLine(SourceAccountingLine sourceLine, KualiDecimal debitAmount,
