...
public class PooledFundValueRule extends MaintenanceDocumentRuleBase{
    ...
    protected boolean checkCustomRequiredFields(PooledFundValue newPooledFundValue) {
        ...
        if (ObjectUtils.isNotNull(incomeDistributionPerUnit) &&
                (incomeDistributionPerUnit.compareTo(zero)==1) ){
            isValid = isIncomeDistributionPerUnitNotNegative(incomeDistributionPerUnit);            
            isValid = isDateRequiredField(distributeIncomeOnDate, EndowPropertyConstants.DISTRIBUTE_INCOME_ON_DATE,
                      EndowKeyConstants.PooledFundValueConstants.ERROR_DISTRIBUTE_INCOME_ON_DATE_IS_REQUIRED_FIELD);            
        }
        ...
        if (ObjectUtils.isNotNull(longTermGainLossDistributionPerUnit) &&
                (longTermGainLossDistributionPerUnit.compareTo(zero)==1)){
            isValid = isDateRequiredField(distributeLongTermGainLossOnDate, EndowPropertyConstants.DISTRIBUTE_LONG_TERM_GAIN_LOSS_ON_DATE,
                      EndowKeyConstants.PooledFundValueConstants.ERROR_DISTRIBUTE_LONG_TERM_GAIN_LOSS_ON_DATE_IS_REQUIRED_FIELD);            
        }        
        ...
        if (ObjectUtils.isNotNull(shortTermGainLossDistributionPerUnit)&&
                (shortTermGainLossDistributionPerUnit.compareTo(zero)==1)){
            isValid = isDateRequiredField(distributeShortTermGainLossOnDate, EndowPropertyConstants.DISTRIBUTE_SHORT_TERM_GAIN_LOSS_ON_DATE,
                      EndowKeyConstants.PooledFundValueConstants.ERROR_DISTRIBUTE_SHORT_TERM_GAIN_LOSS_ON_DATE_IS_REQUIRED_FIELD);            
        }       
...
*/        
    ...
    /**
     * The rule is "A new record cannot have a VAL_DT that is earlier than the record 
     * with the most recent VAL_DT".
     * 
     * @param pooledSecurityID
     * @param theValuationDate
     * @return true if valuationDate is the latest one for the specified pooledSecurityID, 
     *         false otherwise.
     */
    ...
    /**
     * The rule is unitValue can only be positive because unitValue is  a 
     * required field.
     * 
     * @param unitValue 
     ...
     * Either zero or positive value is an accepted/valid input because 
     * incomeDistributionPerUnit is not a required field.
     *     
    ...
    protected boolean isIncomeDistributionPerUnitNotNegative (BigDecimal incomeDistributionPerUnit) {
        ...
        if((incomeDistributionPerUnit.compareTo(new BigDecimal(0))== 1) ||
           (incomeDistributionPerUnit.compareTo(new BigDecimal(0))== 0)){
