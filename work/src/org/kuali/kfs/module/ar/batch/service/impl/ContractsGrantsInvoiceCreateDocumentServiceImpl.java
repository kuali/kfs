...
public class ContractsGrantsInvoiceCreateDocumentServiceImpl implements ContractsGrantsInvoiceCreateDocumentService {
        ...
        if (ObjectUtils.isNotNull(accounts) && !accounts.isEmpty() && !CollectionUtils.isEmpty(accounts)) {
            if (chartOfAccountsCode != null && organizationCode != null) {
                try {
                    ...
                    accountsReceivableDocumentHeader.setProcessingChartOfAccountCode(chartOfAccountsCode);
                    accountsReceivableDocumentHeader.setProcessingOrganizationCode(organizationCode);
    ...
    public Collection<ContractsAndGrantsCGBAward> validateAwards(Collection<ContractsAndGrantsCGBAward> awards, String errOutputFile) {
        ...
        for (ContractsAndGrantsCGBAward award : awards) {
            ...
            if (award.getAwardBeginningDate() != null) {
                ...
                primaryKeys.put("universityDate", award.getAwardBeginningDate());
                ...
                if (universityDate != null) {
                    ...
                    errorList.add(ArConstants.BatchFileSystem.CGINVOICE_CREATION_AWARD_TOO_STALE_ERROR + "Beginning date of award - " + award.getAwardBeginningDate());
                    invalidGroup.put(award, errorList);
                }
            }
            else {
        ...
        if (!CollectionUtils.isEmpty(awardsToBill)) {
            ...
            for (ContractsAndGrantsCGBAward award : awardsToBill) {
                ...
                // 2. Award Invoicing suspended by user.
                ...
                // 5. Award invoicing option is missing
                ...
                // 6. Award preferred billing frequency is not set correctly
                ...
                // 7. Award has no accounts assigned
                ...
                // 8. Award contains expired account or accounts
                ...
                // 9. Award has final invoice Billed already
                ...
                // 10. Award has no valid milestones to invoice
                ...
                // 11. All has no valid bills to invoice
                ...
                // 12. Agency has no matching Customer record
                ...
                // 13. All accounts of an Award have zero$ to invoice
                ...
                // 14. Award does not have appropriate Contract Control Accounts set based on Invoicing Options
                ...
                if (!CollectionUtils.isEmpty(errorString)) {
                    errorList.add(SpringContext.getBean(KualiConfigurationService.class).getPropertyString(errorString.get(0)).replace("{0}", errorString.get(1)));
                    invalidGroup.put(award, errorList);
                ...
                // 16. if there is no AR Invoice Account present when the GLPE is 3. 
                ...
                // 17. If all accounts of award has invoices in progress.
                ...
                // 18. Offset Definition is not available when the GLPE is 3. 
    ...
    public boolean routeContractsGrantsInvoiceDocuments() {
        ...
        try {
            documentIdList = retrieveContractsGrantsInvoiceDocumentsToRoute(KEWConstants.ROUTE_HEADER_SAVED_CD);
        }
    ...
    protected void writeErrorEntryByAward(ContractsAndGrantsCGBAward award, List<String> validationCategory, PrintStream printStream) throws IOException {
        ...
        for (ContractsAndGrantsCGBAwardAccount awardAccount : award.getActiveAwardAccounts()) {
            ...
            cumulativeExpenses = cumulativeExpenses.add(contractsGrantsInvoiceDocumentService.getBudgetAndActualsForAwardAccount(awardAccount, ArPropertyConstants.ACTUAL_BALANCE_TYPE));
    ...
    private List<ContractsAndGrantsCGBAwardAccount> getValidAwardAccounts(List<ContractsAndGrantsCGBAwardAccount> awardAccounts, ContractsAndGrantsCGBAward award) {
        ...
        if (!award.getPreferredBillingFrequency().equalsIgnoreCase(ArPropertyConstants.MILESTONE_BILLING_SCHEDULE_CODE) && !award.getPreferredBillingFrequency().equalsIgnoreCase(ArPropertyConstants.PREDETERMINED_BILLING_SCHEDULE_CODE)) {
            ...
            contractsGrantsInvoiceDocumentService.setAwardAccountToDraw(awdAcctsToCheck);
